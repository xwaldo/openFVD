...
void secnlcsv::saveSection(fstream &file) {
  ...
  for (int i = 0; i < csvNodes.size(); i++) {
    // coordinates in reverse since readVec3() wasn't happy
...
void secnlcsv::loadTrack(QString filename) {
  ...
  while (!file.atEnd()) {
    ...
    if (lineCount) {
      ...
      glm::vec3 pos =
          glm::vec3(lineSplitted[1].toFloat(), lineSplitted[2].toFloat(),
                    lineSplitted[3].toFloat());
      glm::vec3 front =
          glm::vec3(lineSplitted[4].toFloat(), lineSplitted[5].toFloat(),
                    lineSplitted[6].toFloat());
      glm::vec3 left =
          -glm::vec3(lineSplitted[7].toFloat(), lineSplitted[8].toFloat(),
                     lineSplitted[9].toFloat());
