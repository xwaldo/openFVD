...
int secgeometric::updateSection(int node) {
  if (rollFunc->lockedFunc() != -1) {
    if (fabs(rollFunc->funcList.last()->symArg) > 0.00001f &&
        rollFunc->funcList.last()->minArgument * F_HZ < node)
      node = F_HZ * rollFunc->funcList.last()->minArgument - 1.5f;
  }
  if (normForce->lockedFunc() != -1) {
    if (fabs(normForce->funcList.last()->symArg) > 0.00001f &&
        normForce->funcList.last()->minArgument * F_HZ < node)
      node = F_HZ * normForce->funcList.last()->minArgument - 1.5f;
  }
  if (latForce->lockedFunc() != -1) {
    if (fabs(latForce->funcList.last()->symArg) > 0.00001f &&
        latForce->funcList.last()->minArgument * F_HZ < node)
      node = F_HZ * latForce->funcList.last()->minArgument - 1.5f;
  }
  ...
  if (node == 0) {
    ...
    if (bOrientation == 1) {
      diff += glm::dot(lNodes[0].vDir, glm::vec3(0.f, 1.f, 0.f)) *
  ...
  for (int i = 0; i < node; ++i) {
    if (bOrientation == 0) {
      artificialRoll -=
          glm::dot(lNodes[i + 1].vDir, glm::vec3(0.f, -1.f, 0.f)) *
          latForce->getValue((i + 1) / F_HZ) / F_HZ;
    }
  ...
  for (i = node; i < numNodes; i++) {
    ...
    // HACKY: protect against 0 velocity, and ensuing assertion failures
    if (isnanf(prevNode->fVel)) {
    ...
    float pureYawChange =
        (1.f - fabs(glm::dot(curNode->vDir, glm::vec3(0.f, 1.f, 0.f)))) *
        yawChange;
    float pureRollChange =
        glm::dot(curNode->vDir, glm::vec3(0.f, -1.f, 0.f)) * yawChange * F_HZ;
    float deltaAngle =
...
int secgeometric::updateDistanceSection(int node) {
  ...
  while (length < (float)node / F_HZ && i + 1 < lNodes.size()) {
    ...
    if (bOrientation == 0) {
      artificialRoll -= glm::dot(lNodes[i].vDir, glm::vec3(0.f, -1.f, 0.f)) *
...
bool secgeometric::isInFunction(int index, subfunc *func) {
  ...
  if (bArgument == DISTANCE) {
    ...
    return false;
  } else if (index / F_HZ >= func->minArgument &&
             index / F_HZ <= func->maxArgument) {
    return true;
  }
  return false;
}
