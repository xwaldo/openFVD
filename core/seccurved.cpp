...
int seccurved::updateSection(int) {
  ...
  while (fRiddenAngle < fAngle - std::numeric_limits<float>::epsilon()) {
    ...
    if (leadOutNode && fLeadOut > 0.f) {
      if ((fTrans = 1.f - (prevNode->fTotalLength - leadOutNode->fTotalLength) /
    ...
    curNode->vDir =
        glm::angleAxis(TO_RAD(deltaAngle),
                       (float)glm::cos(-fPureDirection * F_PI / 180) *
                               prevNode->vLat +
                           (float)glm::sin(-fPureDirection * F_PI / 180) *
                               prevNode->vNorm) *
        prevNode->vDir;
    curNode->vLat =
        glm::angleAxis(TO_RAD(deltaAngle),
    ...
    if (bOrientation == EULER) {
      ...
      lNodes[numNodes].setRoll(
          glm::dot(lNodes[numNodes].vDir, glm::vec3(0.f, -1.f, 0.f)) *
    ...
    // curNode->vPos =
    // glm::vec3(glm::translate(glm::rotate(glm::translate(vCenter),
    // fAngle/numNodes, (float)glm::cos(-fDirection*F_PI/180) * curNode->vLat  +
    // (float)glm::sin(-fDirection*F_PI/180) * prevNode->vNorm),
    // -vCenter)*glm::vec4(prevNode->vPos, 1));
...
float seccurved::getMaxArgument() { return rollFunc->getMaxArgument(); }
