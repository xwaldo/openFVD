...
func::func(float min, float max, float start, float end, section *_parent,
...
float func::getValue(float x) {
  int i = 0;
  /*while((i < funcList.size()-1 && x > funcList[i+1]->minArgument) || (i <
  funcList.size() && x > funcList[i]->maxArgument)) { i++;
  }*/
...
void func::appendSubFunction(float length, int i) {
  ...
  if (i == -1) {
    ...
    this->funcList.insert(i + 1, new subfunc(pred->maxArgument,
                                             pred->maxArgument + length,
                                             pred->endValue(), 0.f, this));
    activeSubfunc = i + 1;
  }
  const int s = funcList.size();
  for (i = 1; i < s; ++i) {
