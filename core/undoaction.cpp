...
undoAction::undoAction(trackHandler *_track, eActionType _type) {
  ...
  if (type == newTrack || type == deleteTrack)
    ;
  else if (type != changeAnchorPosX && type != changeAnchorPosY &&
           type != changeAnchorPosZ && type != changeAnchorYaw &&
           type != changeAnchorRoll && type != changeAnchorPitch &&
           type != changeAnchorNormal && type != changeAnchorLateral &&
           type != changeAnchorSpeed && type != changeTrackHeartline &&
           type != changeTrackFriction && type != changeTrackResistance &&
           type != changeAnchorPitchChange && type != changeAnchorYawChange) {
    sectionNumber = inTrack->getSectionNumber(inTrack->activeSection);
    if (type != changeSegmentLength && type != changeCurveDirection &&
        type != changeCurveLeadIn && type != changeCurveLeadOut &&
        type != changeCurveRadius && type != changeSpeedState &&
        type != changeSegmentSpeed && type != appendSegment &&
        type != removeSegment && type != changeSegmentArgument &&
        type != changeSegmentOrientation) {
      inFunction = hTrack->graphWidgetItem->selFunc->parent->type;
      subfuncNumber =
          hTrack->graphWidgetItem->selFunc->parent->getSubfuncNumber(
              hTrack->graphWidgetItem->selFunc);
    }
  }
  nextAction = NULL;
  info = NULL;
...
void undoAction::doUndo() {
  if (type == onLengthSpin || type == onChangeSpin || type == onArg1 ||
      type == onCenterSpin || type == onTensionSpin ||
      type == onTransitionBox || type == appendSubFunction ||
      type == changeFunctionStatus) {
    hTrack->trackWidgetItem->setSelection(sectionNumber);
    // parent->on_treeWidget_itemClicked(parent->getTopLevel(sectionNumber+1),
    // 0);
    switch (inFunction) {
    case funcRoll:
      hTrack->graphWidgetItem->changeSelection(
          inTrack->activeSection->rollFunc->funcList[subfuncNumber]);
      break;
    case funcNormal:
      hTrack->graphWidgetItem->changeSelection(
          inTrack->activeSection->normForce->funcList[subfuncNumber]);
      break;
    case funcLateral:
      hTrack->graphWidgetItem->changeSelection(
          inTrack->activeSection->latForce->funcList[subfuncNumber]);
      break;
    case funcPitch:
      hTrack->graphWidgetItem->changeSelection(
          inTrack->activeSection->normForce->funcList[subfuncNumber]);
    ...
    if (subfuncNumber > 0) {
      ...
      case funcRoll:
        hTrack->graphWidgetItem->changeSelection(
            inTrack->activeSection->rollFunc->funcList[subfuncNumber - 1]);
        break;
      case funcNormal:
        hTrack->graphWidgetItem->changeSelection(
            inTrack->activeSection->normForce->funcList[subfuncNumber - 1]);
        break;
      case funcLateral:
        hTrack->graphWidgetItem->changeSelection(
            inTrack->activeSection->latForce->funcList[subfuncNumber - 1]);
        break;
      case funcPitch:
        hTrack->graphWidgetItem->changeSelection(
            inTrack->activeSection->normForce->funcList[subfuncNumber - 1]);
  ...
  } else if (type == changeSegmentLength || type == changeCurveDirection ||
             type == changeCurveLeadIn || type == changeCurveLeadOut ||
             type == changeCurveRadius || type == changeSpeedState ||
             type == changeSegmentSpeed || type == changeSegmentArgument ||
             type == changeSegmentOrientation) {
    hTrack->trackWidgetItem->setSelection(sectionNumber);
    // parent->on_treeWidget_itemClicked(parent->getTopLevel(sectionNumber+1),
    // 0);
  ...
  switch (type) {
  case onLengthSpin:
    hTrack->graphWidgetItem->selFunc->parent->changeLength(fromValue.toDouble(),
                                                           subfuncNumber);
    hTrack->graphWidgetItem->selectionChanged();
    inTrack->updateTrack(
        sectionNumber,
        (int)(hTrack->graphWidgetItem->selFunc->minArgument * F_HZ - 1.5f));
  ...
  case changeSegmentLength:
    inTrack->lSections[sectionNumber]->rollFunc->setMaxArgument(
        fromValue.toDouble());
  ...
  case changeAnchorRoll:
    inTrack->anchorNode->setRoll(fromValue.toDouble() -
                                 inTrack->anchorNode->fRoll);
  ...
  case changeAnchorPitch:
    ...
    inTrack->anchorNode->changePitch((float)fromValue.toDouble() -
                                         inTrack->anchorNode->getPitch(),
                                     fabs(inTrack->anchorNode->fRoll) >= 90.f);
    inTrack->updateTrack(0, 0);
    hTrack->trackWidgetItem->setupAnchorFrame();
    hTrack->graphWidgetItem->redrawGraphs();
    break;
  ...
  case changeAnchorPitchChange:
    hTrack->trackWidgetItem->on_pitchChangeBox_valueChanged(
        fromValue.toDouble() * F_HZ);
    // inTrack->anchorNode->forceNormal = fromValue.toDouble();
    inTrack->updateTrack(0, 0);
    hTrack->trackWidgetItem->setupAnchorFrame();
    hTrack->graphWidgetItem->redrawGraphs();
  ...
  case changeAnchorYawChange:
    hTrack->trackWidgetItem->on_yawChangeBox_valueChanged(fromValue.toDouble() *
                                                          F_HZ);
    // inTrack->anchorNode->forceLateral = fromValue.toDouble();
    inTrack->updateTrack(0, 0);
    hTrack->trackWidgetItem->setupAnchorFrame();
  ...
  case changeSegmentArgument:
    inTrack->lSections.at(sectionNumber)->bArgument =
        !inTrack->lSections.at(sectionNumber)->bArgument;
    inTrack->updateTrack(sectionNumber, 0);
  ...
  case changeSegmentOrientation:
    inTrack->lSections.at(sectionNumber)->bOrientation =
        !inTrack->lSections.at(sectionNumber)->bOrientation;
    inTrack->updateTrack(sectionNumber, 0);
    hTrack->trackWidgetItem->updateOptionsFrame();
    hTrack->graphWidgetItem->redrawGraphs();
  ...
  case changeFunctionStatus:
    ...
    inTrack->updateTrack(
        sectionNumber,
        (int)(hTrack->graphWidgetItem->selFunc->minArgument * F_HZ -
              1.5f)); // SIGSEGV because selFunc was NULL
    hTrack->graphWidgetItem->redrawGraphs();
    break;
  default:
    break;
...
void undoAction::doRedo() {
  if (type == onLengthSpin || type == onChangeSpin || type == onArg1 ||
      type == onCenterSpin || type == onTensionSpin ||
      type == onTransitionBox || type == removeSubFunction ||
      type == changeFunctionStatus) {
    hTrack->trackWidgetItem->setSelection(sectionNumber);
    // parent->on_treeWidget_itemClicked(parent->getTopLevel(sectionNumber+1),
    // 0);
  ...
  switch (type) {
  case onLengthSpin:
    hTrack->graphWidgetItem->selFunc->parent->changeLength(toValue.toDouble(),
                                                           subfuncNumber);
    hTrack->graphWidgetItem->selectionChanged();
    inTrack->updateTrack(
        sectionNumber,
  ...
  case changeAnchorPosZ:
    ...
    break;
  case changeAnchorRoll:
    inTrack->anchorNode->setRoll(toValue.toDouble() -
                                 inTrack->anchorNode->fRoll);
    inTrack->updateTrack(0, 0);
    hTrack->trackWidgetItem->setupAnchorFrame();
    hTrack->graphWidgetItem->redrawGraphs();
  ...
  case changeAnchorPitch:
    ...
    inTrack->anchorNode->changePitch((float)toValue.toDouble() -
                                         inTrack->anchorNode->getPitch(),
                                     fabs(inTrack->anchorNode->fRoll) >= 90.f);
    inTrack->updateTrack(0, 0);
    hTrack->trackWidgetItem->setupAnchorFrame();
  ...
  case changeAnchorLateral:
    ...
    break;
  case changeAnchorPitchChange:
    hTrack->trackWidgetItem->on_pitchChangeBox_valueChanged(toValue.toDouble() *
                                                            F_HZ);
    // inTrack->anchorNode->forceNormal = toValue.toDouble();
    inTrack->updateTrack(0, 0);
    hTrack->trackWidgetItem->setupAnchorFrame();
    ...
    break;
  case changeAnchorYawChange:
    hTrack->trackWidgetItem->on_yawChangeBox_valueChanged(toValue.toDouble() *
                                                          F_HZ);
    // inTrack->anchorNode->forceLateral = toValue.toDouble();
    inTrack->updateTrack(0, 0);
    hTrack->trackWidgetItem->setupAnchorFrame();
