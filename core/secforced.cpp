...
int secforced::updateSection(int node) {
  ...
  for (i = node; i < numNodes; i++) {
    ...
    glm::vec3 forceVec =
        -normForce->getValue((i + 1) / F_HZ) * prevNode->vNorm -
        latForce->getValue((i + 1) / F_HZ) * prevNode->vLat -
    ...
    curNode->setRoll(rollFunc->getValue((i + 1) / F_HZ) /
                     F_HZ); // - rollFunc->getValue(i/1000.f));
    calcDirFromLast(i + 1);
    if (bOrientation == EULER ||
        rollFunc->getSubfunc((i + 1) / F_HZ)->degree == tozero) {
      curNode->setRoll(glm::dot(curNode->vDir, glm::vec3(0.f, -1.f, 0.f)) *
    ...
    curNode->fRollSpeed +=
        rollFunc->getValue((i + 1) / F_HZ); // /1000.f/curNode->fDistFromLast;
