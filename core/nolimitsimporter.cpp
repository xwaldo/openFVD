...
bool noLimitsImporter::importAsTxt() {
  ...
  bList->at(0)->Kp2 =
      2.f / 3.f * bList->at(0)->P1 + 1.f / 3.f * bList->at(1)->P1;
  bList->at(0)->Kp1 = 2.f * bList->at(0)->P1 - bList->at(0)->Kp2;

  bList->last()->Kp1 = 2.f / 3.f * bList->last()->P1 +
                       1.f / 3.f * bList->at(bList->size() - 2)->P1;
  bList->last()->Kp2 = 2.f * bList->last()->P1 - bList->last()->Kp1;

  for (b = 1; b < bList->size() - 1; ++b) {
    bList->at(b)->Kp1 =
        2.f / 3.f * bList->at(b)->P1 + 1.f / 3.f * bList->at(b - 1)->P1;
    bList->at(b)->Kp2 =
        2.f / 3.f * bList->at(b)->P1 + 1.f / 3.f * bList->at(b + 1)->P1;
    bList->at(b)->roll = 0.f;
