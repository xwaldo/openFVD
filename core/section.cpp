...
int section::exportSection(fstream *file, mnode *anchor, float mPerNode,
  ...
  for (int i = 1; i < lNodes.size(); i++) {
    lNodes[i].updateNorm();
    fThreshold += glm::distance(lNodes[i].vPosHeart(fHeart),
...
#define SCALING 3.f
      ...
      if (anchor->vDir.z != 0 || anchor->vDir.x != 0) {
        anchorBase =
            glm::mat4(glm::transpose(glm::mat4(
                          -anchor->vDir.z / temp, 0.f, anchor->vDir.x / temp,
                          0.f, 0.f, 1.f, 0.f, 0.f, -anchor->vDir.x / temp, 0.f,
                          -anchor->vDir.z / temp, 0.f, 0.f, 0.f, 0.f, 1.f)) *
                      glm::mat4(1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f,
                                0.f, 1.f, 0.f, -anchor->fPosHeartx(fHeart),
                                -anchor->fPosHearty(fHeart),
                                -anchor->fPosHeartz(fHeart), 1.f));
      } else {
        anchorBase =
            glm::mat4(glm::transpose(glm::mat4(
                          -anchor->vNorm.z / temp, 0.f, anchor->vNorm.x / temp,
                          0.f, 0.f, 1.f, 0.f, 0.f, -anchor->vNorm.x / temp, 0.f,
                          -anchor->vNorm.z / temp, 0.f, 0.f, 0.f, 0.f, 1.f)) *
                      glm::mat4(1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f,
                                0.f, 1.f, 0.f, -anchor->fPosHeartx(fHeart),
                                -anchor->fPosHearty(fHeart),
                                -anchor->fPosHeartz(fHeart), 1.f));
      ...
      KP1 = anchorBase *
            (glm::vec4(glm::vec3(lNodes[lasti].vPosHeart(fHeart) +
                                 fThreshold / SCALING *
      ...
      KP2 = anchorBase * (glm::vec4(glm::vec3(lNodes[i].vPosHeart(fHeart) -
                                              fThreshold / SCALING *
      ...
      P1 =
          anchorBase * (glm::vec4(glm::vec3(lNodes[i].vPosHeart(fHeart)), 1.f));
      ...
      if (fabs(V.y) < fRollThresh) {
        ...
        glm::vec3 rotateAxis = glm::cross(lNodes[lasti].vDirHeart(fHeart),
                                          lNodes[i].vDirHeart(fHeart));
        glm::vec3 rotated =
            glm::vec3(glm::rotate(glm::angle(lNodes[lasti].vDirHeart(fHeart),
                                             lNodes[i].vDirHeart(fHeart)),
                                  rotateAxis) *
...
void section::fillPointList(QList<glm::vec4> &List, QList<glm::vec3> &Normals,
  ...
  for (int i = 1; i < lNodes.size(); i++) {
    ...
    if (anchor->vDir.z != 0 || anchor->vDir.x != 0) {
      anchorBase = glm::mat4(
          glm::transpose(
              glm::mat4(-anchor->vDir.z / temp, 0.f, anchor->vDir.x / temp, 0.f,
                        0.f, 1.f, 0.f, 0.f, -anchor->vDir.x / temp, 0.f,
                        -anchor->vDir.z / temp, 0.f, 0.f, 0.f, 0.f, 1.f)) *
          glm::mat4(1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f,
                    -anchor->fPosHeartx(fHeart), -anchor->fPosHearty(fHeart),
                    -anchor->fPosHeartz(fHeart), 1.f));
    } else {
      anchorBase = glm::mat4(
          glm::transpose(
              glm::mat4(-anchor->vNorm.z / temp, 0.f, anchor->vNorm.x / temp,
                        0.f, 0.f, 1.f, 0.f, 0.f, -anchor->vNorm.x / temp, 0.f,
                        -anchor->vNorm.z / temp, 0.f, 0.f, 0.f, 0.f, 1.f)) *
          glm::mat4(1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f,
                    -anchor->fPosHeartx(fHeart), -anchor->fPosHearty(fHeart),
...
void section::iFillPointList(QList<int> &List, float mPerNode) {
  ...
  if (List.size() > 1 &&
      (lNodes[List.last() - parent->getNumPoints(this) + 1].fTotalLength -
...
void section::fFillPointList(QList<int> &List, float mPerNode) {
  ...
  if (this->type == straight && this->rollFunc->funcList.size() == 1 &&
      this->rollFunc->funcList[0]->symArg == 0.f) {
    if (List.size()) {
      List.last() *= -1;
    }
...
void section::calcDirFromLast(int i) {
  ...
  if (diff.length() <= std::numeric_limits<float>::epsilon()) {
    ...
    lNodes[i].fYawFromLast =
        lNodes[i].getDirection() - lNodes[i - 1].getDirection();
    lNodes[i].fDirFromLast =
        glm::atan(lNodes[i].fYawFromLast, lNodes[i].fPitchFromLast) * 180.f /
            F_PI -
        lNodes[i].fRoll;
  }
  ...
  float fTrackPitchFromLast =
      180.f / F_PI * (asin(curDirHeart.y) - asin(prevDirHeart.y));
  float fTrackYawFromLast = 180.f / F_PI *
