...
float interpolate(float t, float x1, float x2) { return t * x2 + x1 - t * x1; }
...
float interpolate(float t, float x1, float x2, float x3, float x4) {
  float t1 = 1 - t;
  return t1 * t1 * t1 * x1 + 3 * t * t1 * t1 * x2 + 3 * t * t * t1 * x3 +
         t * t * t * x4;
}

subfunc::subfunc() {}

subfunc::subfunc(float min, float max, float start, float diff,
                 func *getparent) {
  minArgument = min;
  maxArgument = max;
...
void subfunc::updateBez() {
  ...
  while (i < 100) {
    ...
    t += (nextT - gotT) /
         (3 * interpolate(t, pointList[0].x, pointList[1].x - pointList[0].x,
                          1.f - pointList[1].x));
    ++i;
  }
  valueList.append(1);
...
float subfunc::getValue(float x) {
  ...
  case quartic:
    if (!isSymmetric()) {
      return x * x *
                 (-(6 * symArg * arg1) / (1 - 2 * arg1) +
                  x * (symArg * (4 * arg1 + 4) / (1 - 2 * arg1) +
                       x * ((-3 * symArg / (1 - 2 * arg1))))) +
             startValue;
    } else {
                ...
                (-0.0288 - 0.00448 * root +
                 fabs(arg1 / 10.f) *
                     (0.0032 - 0.00576 * root +
                      fabs(arg1 / 10.f) *
                          (-0.0704 + 0.02048 * root +
                           fabs(arg1 / 10.f) * (0.1024 - 0.01024 * root +
                                                arg1 / 10.f * 0.04096))));
      return symArg / max * x * x * (x - 1) * (x - 1) * (x + arg1 / 10.f) +
             startValue;
    } else {
  ...
  case plateau:
    ...
    return symArg * (1.f - (exp(-arg1 * 15.f *
                                (pow(1.f - fabs(2.f * x - 1.f), 3))))) +
           startValue;
    break;
  case freeform:
    ...
    if ((int)max == valueList.size() - 1) {
      ...
      return (1 - root) * symArg * valueList[(int)max] +
  ...
  case tozero:
    ...
    curNode = this->parent->secParent->parent->getPoint(
        inTrack->getNumPoints(parent->secParent) + minArgument * 1000.f - 0.5f);
    prevNode = this->parent->secParent->parent->getPoint(
        inTrack->getNumPoints(parent->secParent) + minArgument * 1000.f - 1.5f);
    if (this->parent->secParent->bOrientation == EULER) {
      ...
      d = (curNode->fRollSpeed +
           ...
           glm::dot(prevNode->vDir, glm::vec3(0.f, -1.f, 0.f)) *
               prevNode->fYawFromLast) *
          F_HZ;
      e = -glm::dot(curNode->vDir, glm::vec3(0.f, -1.f, 0.f)) *
