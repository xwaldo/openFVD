...
mnode::mnode(glm::vec3 getPos, glm::vec3 getDir, float getRoll, float getVel,
             float getNForce, float getLateral) {
  this->vPos = getPos;
  this->vDir = getDir;
  ...
  if (this->vDir.y == 1) {
    this->vLat =
        glm::vec3(glm::angleAxis(TO_RAD(getRoll), glm::vec3(0.f, -1.f, 0.f)) *
                  glm::vec4(1.f, 0.f, 0.f, 0.f));
  } else {
    this->vLat = glm::vec3(-this->vDir.z, 0.f, this->vDir.x);
  }

  this->vLat.y = tan(fRoll * F_PI / 180) * sqrt(this->vLat.x * this->vLat.x +
...
void mnode::changeYaw(float dAngle) {
  vDir = glm::normalize(
      glm::angleAxis(TO_RAD(dAngle), glm::vec3(0.f, 1.f, 0.f)) * vDir);
  vLat = glm::normalize(
      glm::angleAxis(TO_RAD(dAngle), glm::vec3(0.f, 1.f, 0.f)) * vLat);
  this->updateNorm();
}
...
glm::vec3 mnode::vLatHeart(float fHeart) {
  ...
  float fRollSpeedPerMeter =
      estDistFromLast > 0.f ? (fRollSpeed + fSmoothSpeed) / F_HZ / estimated
                            : 0.f;
  return glm::normalize(glm::normalize(vLat) -
                        glm::normalize(vDir) * (float)(fRollSpeedPerMeter *
                                                       F_PI * fHeart / 180.f));
}
...
glm::vec3 mnode::vDirHeart(float fHeart) {
  ...
  float fRollSpeedPerMeter =
      fHeartDistFromLast > 0.f ? (fRollSpeed + fSmoothSpeed) / F_HZ / estimated
                               : 0.f;
  if (fRollSpeedPerMeter != fRollSpeedPerMeter)
    fRollSpeedPerMeter = 0.f;
  return glm::normalize(
      vDir + vLat * (float)(fRollSpeedPerMeter * F_PI * fHeart / 180.f));
}

void mnode::exportNode(QList<bezier_t *> &bezList, mnode *last, mnode *,
                       mnode *anchor, float fHeart, float fRollThresh) {
#define SCALING 3.f
  ...
  float angle =
      (F_HZ * this->fTrackAngleFromLast * F_PI / 180.f) / this->fVel * realDist;
  ...
  if (bezList.size() > 1) {
    bezList.last()->Kp1 = bezList[bezList.size() - 2]->P1 +
  ...
  bezList.last()->Kp2 =
      bezList.last()->P1 - anchorBase * (fThreshold * this->vDirHeart(fHeart));
  ...
  if (fabs(this->vDirHeart(fHeart).y) < fRollThresh) {
    ...
    glm::vec3 rotated =
        glm::vec3(glm::rotate(glm::angle(last->vDirHeart(fHeart),
                                         this->vDirHeart(fHeart)),
                              rotateAxis) *
                  glm::vec4(last->vLatHeart(fHeart), 0.f));
...
void mnode::calcSmoothForces() {
  ...
  if (fabs(fAngleFromLast) < std::numeric_limits<float>::epsilon()) {
    ...
    float normalDAngle = F_PI / 180.f *
                         (-fPitchFromLast * cos(fRoll * F_PI / 180.) -
                          temp * fYawFromLast * sin(fRoll * F_PI / 180.));
    float lateralDAngle = F_PI / 180.f *
                          (fPitchFromLast * sin(fRoll * F_PI / 180.) -
                           temp * fYawFromLast * cos(fRoll * F_PI / 180.));
    forceVec = glm::vec3(0.f, 1.f, 0.f) +
