...
public:
  mnode();
  mnode(glm::vec3 getPos, glm::vec3 getDir, float getRoll, float getVel,
  ...
  glm::vec3 vRelPos(float y, float x, float z = 0.f) {
    return vPos - y * vNorm + x * vLatHeart(-y) + z * vDirHeart(-y);
  }

  void exportNode(QList<bezier_t *> &bezList, mnode *last, mnode *mid,
  ...
  float fSmoothSpeed;
  float fFlexion() {
    return fDistFromLast <= 0.0 ? 0.0f : fTrackAngleFromLast / fDistFromLast;
  }
  float fTotalLength;
