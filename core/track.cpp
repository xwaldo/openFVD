...
void track::removeSmooth(int fromNode) {
  ...
  for (int i = 0; i < lSections.size(); ++i) {
    ...
    if (fromNode >= curSection->lNodes.size() &&
...
void track::updateTrack(int index, int iNode) {
  ...
  gloParent->showMessage(
      QString::number(mSec).append(
          QString("ms used to update %1 (%2) points").arg(count2).arg(count)),
      3000);
...
void track::newSection(enum secType type, int index) {
  ...
  if (index == -1) {
    ...
    smoothList.insert(lSections.size(),
                      new smoothHandler(this, lSections.size() - 1));
  } else if (index == 0) {
    lSections.prepend(newSection);
    newSection->updateSection();
    if (lSections.size() > 1) {
      lSections.at(1)->lNodes.prepend(
          newSection->lNodes[newSection->lNodes.size() - 1]);
    }
    ...
    if (lSections.size() > index + 1) {
      lSections.at(index + 1)->lNodes.prepend(
          newSection->lNodes[newSection->lNodes.size() - 1]);
    }
...
int track::exportTrack(fstream *file, float mPerNode, int fromIndex,
  ...
  bezList.last()->Kp2 =
      (bezList.last()->P1 - bezList.last()->Kp1) + bezList.last()->P1;
...
int track::exportTrack2(fstream *file, float mPerNode, int fromIndex,
                        int toIndex, float fRollThresh) {
  QList<int> exportPoints;
  mnode *anchor = &lSections.at(fromIndex)->lNodes[0];
  ...
  KP2_this = anchor->vDirHeart(fHeart) *
             glm::distance(anchor->vPosHeart(fHeart),
                           getPoint(exportPoints[0])->vPosHeart(fHeart)) /
             3.f;
  ...
  glm::vec3 startPoint =
      getPoint(exportPoints[0])->vPosHeart(fHeart) +
      (KP2_this + anchorPos - getPoint(exportPoints[0])->vPosHeart(fHeart)) /
          2.f * 3.f;
  qDebug("startPoint %f %f %f", startPoint.x, startPoint.y, startPoint.z);

  P = glm::vec3((1 / 6.f) *
                (startPoint +
                 4.f * getPoint(exportPoints[0])->vPosHeart(fHeart) +
                 getPoint(exportPoints[1])->vPosHeart(fHeart))) -
      anchorPos;
  KP1_this = glm::vec3((1 / 3.f) *
                       (startPoint +
                        2.f * getPoint(exportPoints[0])->vPosHeart(fHeart))) -
             anchorPos;
  ...
  glm::vec3 vHeartLat =
      glm::normalize(glm::cross(getPoint(exportPoints[0])->vNorm, V));
  ...
  for (i = 1; i < exportPoints.size() - 2; ++i) {
    ...
    P = glm::vec3((1 / 6.f) *
                  (getPoint(exportPoints[i - 1])->vPosHeart(fHeart) +
                   4.f * getPoint(exportPoints[i])->vPosHeart(fHeart) +
                   getPoint(exportPoints[i + 1])->vPosHeart(fHeart))) -
        anchorPos;
    KP1_this = glm::vec3((1 / 3.f) *
                         (getPoint(exportPoints[i - 1])->vPosHeart(fHeart) +
    ...
    glm::vec3 vHeartLat =
        glm::normalize(glm::cross(getPoint(exportPoints[i])->vNorm, V));
    ...
    if (fabs(V.y) > fRollThresh) {
      ...
      glm::vec3 rotateAxis =
          glm::cross(getPoint(exportPoints[i - 1])->vDirHeart(fHeart), V);
      glm::vec3 rotated = glm::vec3(
          glm::rotate(
              glm::angle(getPoint(exportPoints[i - 1])->vDirHeart(fHeart), V),
              rotateAxis) *
          glm::vec4(vLastLat, 0.f));
      temp = glm::angle(rotated, vHeartLat) * F_PI / 180.f;
      if (glm::dot(glm::cross(rotated, vHeartLat), V) > 0) {
  ...
  glm::vec3 endPoint =
      getPoint(exportPoints[exportPoints.size() - 1])->vPosHeart(fHeart) -
      getPoint(exportPoints[exportPoints.size() - 1])->vDirHeart(fHeart) *
          glm::distance(getPoint(exportPoints[exportPoints.size() - 1])
                            ->vPosHeart(fHeart),
                        getPoint(exportPoints[exportPoints.size() - 2])
                            ->vPosHeart(fHeart));
  ...
  P = glm::vec3((1 / 6.f) *
      ...
      anchorPos;
  KP1_this =
      glm::vec3((1 / 3.f) * (getPoint(exportPoints[i - 1])->vPosHeart(fHeart) +
                             2.f * endPoint)) -
      anchorPos;
  KP2_this = glm::vec3((1 / 3.f) *
                       (2.f * endPoint +
  ...
  P = getPoint(exportPoints[exportPoints.size() - 1])->vPosHeart(fHeart) -
      anchorPos;
  KP1_this =
      P - getPoint(exportPoints[exportPoints.size() - 1])->vDirHeart(fHeart) *
              glm::distance(getPoint(exportPoints[exportPoints.size() - 1])
                                ->vPosHeart(fHeart),
                            getPoint(exportPoints[exportPoints.size() - 2])
                                ->vPosHeart(fHeart)) /
...
// spline export
int track::exportTrack3(fstream *file, float mPerNode, int fromIndex,
                        int toIndex, float fRollThresh) {
  QList<int> exportPoints;
  mnode *anchor = &lSections.at(fromIndex)->lNodes[0];
  for (int i = fromIndex; i <= toIndex; ++i) {
...
// tangent export
int track::exportTrack4(fstream *file, float mPerNode, int fromIndex,
                        int toIndex, float fRollThresh) {
  QList<int> exportPoints;
  mnode *anchor = &lSections.at(fromIndex)->lNodes[0];
  for (int i = fromIndex; i <= toIndex; ++i) {
  ...
  mnode *last = anchor, *current = getPoint(exportPoints[0]),
        *mid = getPoint(exportPoints[0] / 2);
  QList<bezier_t *> bezList;

  for (int i = 0; i < exportPoints.size(); ++i) {
    current = getPoint(exportPoints[i]);

    mid = NULL; // i == 0 ? getPoint(exportPoints[0]/2) :
                // getPoint((exportPoints[i]+exportPoints[i-1])/2);
...
void track::exportNL2TrackCSV(FILE *file, float mPerNode, int fromIndex,
                              int toIndex) {
  QList<int> exportPoints, rollPoints;
  mnode *anchor = &lSections.at(fromIndex)->lNodes[0];
  exportPoints.append(getNumPoints(lSections.at(fromIndex)));
  ...
  for (size_t i = 1; i < size - 1; ++i) {
    ...
    if (lSections[0]->type != 6) {
      ...
      // if strict == 3 -> export normally
      // if strict == 4 -> open section into straight -> resolve point
      // if strict == 5 -> straight into straight -> resolve point into two
      // points if strict == 6 -> export normally if strict == 7 -> can't happen
      ...
      if (strict == 1) {
        ...
        e.append(glm::vec4(getPoint(ppoint)->vPos - x0 * getPoint(ppoint)->vDir,
                           0.f));
        e.append(glm::vec4(getPoint(npoint)->vPos + x0 * getPoint(npoint)->vDir,
                           0.f));
  ...
  if (lSections[0]->type == 6) {
    offset.x = lSections[0]->lNodes[0].vPos.x;
    offset.y = lSections[0]->lNodes[0].vPos.y;
    offset.z = lSections[0]->lNodes[0].vPos.z;
  }

  for (int i = 0; i < e.size(); ++i) {
    glm::vec3 ex = glm::vec3(e[i]);

    int point = exportPoints[i];
    mnode *curNode = getPoint(point < 0 ? -point : point);

    glm::vec3 front = curNode->vDir;
    glm::vec3 left = -curNode->vLat;
    glm::vec3 up = -curNode->vNorm;

    fprintf(file, "%d\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\n", i + 1,
            ex.x + offset.x, ex.y + offset.y, ex.z + offset.z, front.x, front.y,
            front.z, left.x, left.y, left.z, up.x, up.y, up.z);
  }
...
QString track::saveTrack(fstream &file, trackWidget *_widget) {
  ...
  writeBytes(&file, (const char *)&(_widget->inTrack->trackColors),
             3 * sizeof(QColor));
...
QString track::loadTrack(fstream &file, trackWidget *_widget) {
  ...
  size = readInt(&file);
  for (int i = 0; i < size; ++i) {
    if (i >= smoothList.size())
      smoothList.append(new smoothHandler(this, -2));
