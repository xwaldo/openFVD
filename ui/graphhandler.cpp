...
graphHandler::graphHandler(QTreeWidgetItem *_treeItem, enum graphType _type,
                           trackHandler *_track, QCPAxis *_axis,
                           QColor *_color) {
  treeItem = _treeItem;
  active = false;
  drawn = false;
...
void graphHandler::fillGraphList(QCPAxis *xAxis, bool _argument,
                                 bool _orientation, bool _drawExterns) {
  ...
  if (mType == secBoundaries) {
    if (mTrack->trackData->activeSection)
      fillBoundaryGraphList(xAxis, _argument);
    return;
  }
  ...
  for (int i = 0; i < mTrack->trackData->lSections.size(); ++i) {
    ...
    y.clear();
    if (active &&
        mTrack->trackData->activeSection == mTrack->trackData->lSections[i]) {
      fillActiveGraphList(xAxis, _argument, _drawExterns);
      continue;
    } else if (!_drawExterns && mTrack->trackData->activeSection !=
                                    mTrack->trackData->lSections[i]) {
      continue;
    }
     ...
     */
    if (mType == nForceChange || mType == lForceChange || mType == banking ||
        mType == rollAccel) {
    ...
    } else if (mType == smoothedRollSpeed || mType == smoothedNForce ||
               mType == smoothedLForce) {
      QColor temp = color[0];
      ...
      if (_argument == TIME) {
        ...
        case rollSpeed:
          if (_orientation == QUATERNION) {
            ...
            y.append(curNode->fRollSpeed -
                     glm::dot(curNode->vDir, glm::vec3(0.f, -1.f, 0.f)) *
                         curNode->getYawChange());
          }
          break;
        ...
        case rollAccel:
          if (_orientation == QUATERNION) {
            y.append((curNode->fRollSpeed + curNode->fSmoothSpeed -
                      prevNode->fRollSpeed - prevNode->fSmoothSpeed) *
                     F_HZ / diff);
          } else {
            y.append((curNode->fRollSpeed + curNode->fSmoothSpeed -
                      prevNode->fRollSpeed - prevNode->fSmoothSpeed -
                      glm::dot(curNode->vDir, glm::vec3(0.f, -1.f, 0.f)) *
                          curNode->getYawChange() +
                      glm::dot(prevNode->vDir, glm::vec3(0.f, -1.f, 0.f)) *
                          prevNode->getYawChange()) *
                     F_HZ / diff);
          }
          break;
        case nForce:
        ...
        case nForceChange:
          y.append((curNode->forceNormal + curNode->smoothNormal -
                    prevNode->forceNormal - prevNode->smoothNormal) *
                   F_HZ / diff);
          break;
        case lForce:
        ...
        case lForceChange:
          y.append((curNode->forceLateral + curNode->smoothLateral -
                    prevNode->forceLateral - prevNode->smoothLateral) *
                   F_HZ / diff);
          break;
        case pitchChange:
...
void graphHandler::fillActiveGraphList(QCPAxis *xAxis, bool _argument,
                                       bool _drawExterns) {
  ...
  for (int i = 0; i < func->funcList.size(); ++i) {
    ...
    if (curFunc->locked)
      curFunc->getValue(-1.f); // make sure maxArg is right
    ...
    double key;
    if (curTrack->activeSection->type == straight ||
        curTrack->activeSection->type == curved || curFunc->degree == tozero) {
      ...
      if (curTrack->activeSection->isInFunction(r, curFunc))
        l = r;
      while (l < r) {
      ...
      for (unsigned int j = 0; j < max_segs_per_active_transition + 1; ++j) {
        key = (upper * j + lower * (max_segs_per_active_transition - j)) /
              (double)max_segs_per_active_transition;
        ...
        case rollSpeed:
          if (curTrack->activeSection->bOrientation == EULER) {
            y.append(curTrack->activeSection->lNodes[key].fRollSpeed +
                     sin(curTrack->activeSection->lNodes[key].getPitch() *
                         F_PI / 180.) *
                         curTrack->activeSection->lNodes[key].getYawChange());
          } else {
            y.append(curTrack->activeSection->lNodes[key].fRollSpeed);
          }
          break;
      ...
      for (int j = 0; j < 251; ++j) {
        double maxArg;
        if (curTrack->activeSection == curTrack->lSections.last() ||
            !_drawExterns) {
          maxArg = curFunc->maxArgument;
        } else {
          maxArg =
              curTrack->activeSection->getMaxArgument() < curFunc->maxArgument
                  ? curTrack->activeSection->getMaxArgument()
                  : curFunc->maxArgument;
        }
