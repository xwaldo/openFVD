...
TrackProperties::TrackProperties(QWidget *parent)
    : QDialog(parent), ui(new Ui::TrackProperties) {
  ui->setupUi(this);
  curTrack = NULL;
...
void TrackProperties::on_buttonBox_accepted() {
  ...
  if (curTrack->trackData->fHeart != ui->heartlineBox->value()) {
    ...
    float x = -cos(yaw) * temp.z + sin(yaw) * temp.x +
              curTrack->trackData->startPos.x;
    float y = temp.y + curTrack->trackData->startPos.y;
    float z =
        cos(yaw) * temp.x + sin(yaw) * temp.z + curTrack->trackData->startPos.z;
    ...
    curTrack->trackData->startPos.x =
        -(cos(yaw) * sin(pitch) * cos(roll) + sin(yaw) * sin(roll)) *
            curTrack->trackData->fHeart +
        x;
    curTrack->trackData->startPos.y =
        (cos(pitch) * cos(roll)) * curTrack->trackData->fHeart + y;
    curTrack->trackData->startPos.z =
  ...
  if (curTrack->trackData->style !=
      (enum trackStyle)ui->styleBox->currentIndex()) {
