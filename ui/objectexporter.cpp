...
objectExporter::objectExporter(QWidget *parent)
    : QDialog(parent), ui(new Ui::objectExporter) {
  ui->setupUi(this);
  curTrackIndex = -1;
}

objectExporter::~objectExporter() { delete ui; }
...
void objectExporter::on_buttonBox_accepted() {
  QString fileName = QFileDialog::getSaveFileName(
      gloParent, "Save 3ds Object", ".", "3D Object (*.3ds)", 0, 0);
  ...
  {
    for (int section = 0; section < curTrack->trackData->lSections.size();
         ++section) {
      vertices->clear();
      indices->clear();
      borders->clear();
      ...
      for (int subIndex = 0; subIndex < borders->size() - 2; subIndex += 2) {
        ...
        int i, j;
        QString name = QString::number(section).append(
            QString("_").append(QString::number(subIndex / 2)));
        ...
        for (i = 0; i < toVIndex - fromVIndex; ++i) {
          lib3ds_vector_copy(mesh->vertices[i],
                             &fvertices[(i + fromVIndex) * 3]);
          mesh->texcos[i][0] = 0.f;
          mesh->texcos[i][1] = 0.f;
        }
        ...
        for (i = 0; i < toIIndex - fromIIndex; ++i) {
          for (j = 0; j < 3; ++j) {
            mesh->faces[i].index[j] =
                indices->at(3 * (i + fromIIndex) + j) - fromVIndex;
            mesh->faces[i].material = 0;
          }
        }
        inst = lib3ds_node_new_mesh_instance(mesh, name.toLocal8Bit().data(),
                                             NULL, NULL, NULL);
...

  {
    ...
    for (i = 0; i <= 36; i++) {
      ...
      lib3ds_vector_make(n->pos_track.keys[i].value,
                         ...
                         (float)(100.0 * sin(2 * F_PI * i / 36.0)), 50.0);
    }
  }
