...
exportUi::exportUi(QWidget *parent) : QDialog(parent), ui(new Ui::Exportui) {
  ui->setupUi(this);
}

exportUi::~exportUi() { delete ui; }

exportUi::exportUi(QWidget *parent, projectWidget *_project)
    : QDialog(parent), ui(new Ui::Exportui) {
  phantomChanges = true;
  ui->setupUi(this);
  ...
  for (int i = 0; i < project->trackList.size(); ++i) {
    ui->exportTrackBox->addItem(project->trackList[i]->trackData->name +
  ...
  if (project->selTrack) {
    ...
    for (int i = 0; i < tTrack->lSections.size(); ++i) {
      ui->exportFromBox->addItem(tTrack->lSections[i]->sName +
                                     QString(" (" + QString().number(i + 1)) +
                                     QString(")"),
                                 QVariant().fromValue(i));
      ui->exportToBox->addItem(tTrack->lSections[i]->sName +
                                   QString(" (" + QString().number(i + 1)) +
                                   QString(")"),
                               QVariant().fromValue(i));
    }
...
void exportUi::doExport() {
  ...
  if (!fileName.isEmpty()) {
    ...
    int iNodes = tTrack->exportTrack4(fout, this->fPerNode, curFromIndex,
                                      curToIndex + curFromIndex, fRollThresh);
    ...
    fout->seekp(4);
    writeBytes(fout, (const char *)&iDataLength,
               4); // replaced with length of data
    fout->seekp(72);
    writeBytes(fout, (const char *)&iNodes,
               4); // replaced with no of NL beziers

    fout->close();
    delete fout;
    gloParent->backupSave();
    gloParent->displayStatusMessage(
        QString("Export to ").append(fileName).append(" successful!"));
  } else {
...
void exportUi::doExport2() {
  ...
  if (!fileName.isEmpty()) {
    ...
    int iNodes = tTrack->exportTrack3(fout, this->fPerNode, curFromIndex,
                                      curToIndex + curFromIndex, fRollThresh);
...
void exportUi::doNL2Export() {
  ...
  if (!fileName.isEmpty()) {
    ...
    tTrack->exportNL2Track(fout, fPerNode, curFromIndex,
...
void exportUi::doNL2ExportCSV() {
  ...
  if (!fileName.isEmpty()) {
    ...
    fprintf(
        fout,
        "No.\tPosX\tPosY\tPosZ\tFrontX\tFrontY\tFrontZ\tLeftX\tLeftY\tLeftZ\t"
        "UpX\tUpY\tUpZ\n");

    tTrack->exportNL2TrackCSV(fout, fPerNode, curFromIndex,
                              curFromIndex + curToIndex);
...
void exportUi::on_buttonBox_accepted() {
  ...
  } else if (ui->exportTypeBox->currentIndex() == 1) {
    filter = QString("CSV (*.csv)");
  } else {
...
void exportUi::on_buttonBox_rejected() { this->close(); }
...
bool exportUi::updateBoxes() {
  ...
  if (curFromIndex == -1 && curToIndex == -1) {
    ...
    if (curToIndex >= tTrack->lSections.size() ||
        curFromIndex >= tTrack->lSections.size()) {
      ui->exportFromBox->setCurrentIndex(0);
      ui->exportToBox->setCurrentIndex(tTrack->lSections.size() - 1);
      curFromIndex = 0;
      curToIndex = tTrack->lSections.size() - 1;
