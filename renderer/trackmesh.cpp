...
QMap<QString,
     std::tuple<QVector<glm::vec3>, QVector<glm::ivec3>, QVector<glm::ivec3>,
                QVector<glm::ivec3>, QVector<glm::vec3>, QVector<glm::vec2>>>
    trackMesh::objCache;
void trackMesh::loadObj(QVector<tracknode_t> &list,
                        QVector<meshnode_t> &shadowlist,
                        const QString &filePath, mnode *curNode, float fHeart,
                        float scale) {
  if (!objCache.contains(filePath)) {
    QFile file(filePath);
    ...
    QVector<glm::ivec3> normalIndices;
    QVector<glm::ivec3> texCoordIndices;
    ...
    while (!in.atEnd()) {
      ...
      if (tokens[0] == "v") {
        vertices.append(glm::vec3(tokens[1].toFloat(), tokens[2].toFloat(),
                                  tokens[3].toFloat()));
      } else if (tokens[0] == "vn") {
        normals.append(glm::vec3(tokens[1].toFloat(), tokens[2].toFloat(),
                                 tokens[3].toFloat()));
      } else if (tokens[0] == "vt") {
        texCoords.append(glm::vec2(tokens[1].toFloat(), tokens[2].toFloat()));
      } else if (tokens[0] == "f") {
        QVector<int> vertexIndices, normalIndicesLocal, texCoordIndicesLocal;
        for (int i = 1; i < tokens.size(); ++i) {
          QStringList parts = tokens[i].split("/");
          vertexIndices.append(parts[0].toInt() - 1);
          texCoordIndicesLocal.append(parts.size() > 1 && !parts[1].isEmpty()
                                          ? parts[1].toInt() - 1
                                          : -1);
          normalIndicesLocal.append(parts.size() > 2 && !parts[2].isEmpty()
                                        ? parts[2].toInt() - 1
                                        : -1);
        }

        faces.append(
            glm::ivec3(vertexIndices[0], vertexIndices[1], vertexIndices[2]));
        normalIndices.append(glm::ivec3(normalIndicesLocal[0],
                                        normalIndicesLocal[1],
                                        normalIndicesLocal[2]));
        texCoordIndices.append(glm::ivec3(texCoordIndicesLocal[0],
                                          texCoordIndicesLocal[1],
                                          texCoordIndicesLocal[2]));

        if (tokens.size() > 4) // Handle quads by splitting into two triangles
        {
          faces.append(
              glm::ivec3(vertexIndices[2], vertexIndices[3], vertexIndices[0]));
          normalIndices.append(glm::ivec3(normalIndicesLocal[2],
                                          normalIndicesLocal[3],
                                          normalIndicesLocal[0]));
          texCoordIndices.append(glm::ivec3(texCoordIndicesLocal[2],
                                            texCoordIndicesLocal[3],
                                            texCoordIndicesLocal[0]));
        }
      }
    }
    file.close();

    objCache[filePath] = std::make_tuple(vertices, faces, normalIndices,
                                         texCoordIndices, normals, texCoords);
  }
  float isInvert = abs(scale) / scale;
  auto [vertices, faces, normalIndices, texCoordIndices, normals, texCoords] =
      objCache[filePath];

  QVector<glm::vec3> transformedVertices;
  for (const glm::vec3 &vertex : vertices) {
    transformedVertices.append(curNode->vRelPos(
        vertex.y * isInvert - fHeart, vertex.x * scale, vertex.z * scale));
  }

  QVector<glm::vec3> adjustedNormals;
  for (const glm::vec3 &normal : normals) {
    glm::vec3 adjustedNormal =
        curNode->vDir * normal.z + curNode->vLat * normal.x +
        glm::cross(curNode->vLat, curNode->vDir) * normal.y;
    adjustedNormals.append(glm::normalize(adjustedNormal));
  }

  for (int i = 0; i < faces.size(); ++i) {
    const glm::ivec3 &face = faces[i];
    glm::vec3 P1 = transformedVertices[face.x];
    glm::vec3 P2 = transformedVertices[face.y];
    glm::vec3 P3 = transformedVertices[face.z];

    glm::vec3 N1 = (normalIndices[i].x >= 0)
                       ? adjustedNormals[normalIndices[i].x]
                       : glm::vec3(0.0f, 1.0f, 0.0f);
    glm::vec3 N2 = (normalIndices[i].y >= 0)
                       ? adjustedNormals[normalIndices[i].y]
                       : glm::vec3(0.0f, 1.0f, 0.0f);
    glm::vec3 N3 = (normalIndices[i].z >= 0)
                       ? adjustedNormals[normalIndices[i].z]
                       : glm::vec3(0.0f, 1.0f, 0.0f);

    glm::vec2 T1 = (texCoordIndices[i].x >= 0) ? texCoords[texCoordIndices[i].x]
                                               : glm::vec2(0.0f, 0.0f);
    glm::vec2 T2 = (texCoordIndices[i].y >= 0) ? texCoords[texCoordIndices[i].y]
                                               : glm::vec2(0.0f, 0.0f);
    glm::vec2 T3 = (texCoordIndices[i].z >= 0) ? texCoords[texCoordIndices[i].z]
                                               : glm::vec2(0.0f, 0.0f);

    nextNorm = N1 * isInvert;
    nextPos = P1;
    appendTrackNode(list, T1.x, T1.y);
    nextNorm = N2 * isInvert;
    nextPos = P2;
    appendTrackNode(list, T2.x, T2.y);
    nextNorm = N3 * isInvert;
    nextPos = P3;
    appendTrackNode(list, T3.x, T3.y);

    glm::vec3 lightvec = glm::vec3(0.f, 1.f, 0.f);
    glm::vec3 triNormal = glm::normalize(glm::cross(P3 - P1, P2 - P1));
    float dotVal = glm::dot(triNormal, lightvec);

    if (dotVal <= 0) {
      float shadowY = -1.f - trackData->startPos.y;
      glm::vec3 PS1(P1.x, shadowY, P1.z);
      glm::vec3 PS2(P2.x, shadowY, P2.z);
      glm::vec3 PS3(P3.x, shadowY, P3.z);

      createQuad(shadowlist, P1, P2, PS2, PS1);
      createQuad(shadowlist, P2, P3, PS3, PS2);
      createQuad(shadowlist, P3, P1, PS1, PS3);

      createShadowTriangle(shadowlist, P1, P2, P3);
    }
  }
}
...
void trackMesh::buildMeshes(int fromNode) {
  ...
  float defaultGauge = 1.0f;
  float distFromLastNode = 0.f;
  float fHeart = trackData->fHeart;
  float isInvert = (fHeart < 0 ? -1.f : 1.f);
  float spineOffset = 0.2f;
  float spineHeight = 1.0f;
  float spineWidth = 1.0f;
  ...
  spineHeight *= isInvert;
  spineOffset *= isInvert;
  spineWidth /= 2;
  float spineBottom = spineOffset + spineHeight;
  defaultGauge /= 2;
  railSpacing = useGauge ? railSpacing : defaultGauge;
  float scale = railSpacing / defaultGauge * isInvert;
  ...
  pipeoption_t temp;
  temp.edges = 12;
  temp.offset = glm::vec2(-railSpacing, -fHeart);
  ...
  case box:
    ...
    options.append(temp);
    // createPipe(rails, 12, spineSize, spineSize,
    // -fHeart-spineOffset, -railSpacing); createPipe(rails, 12,
    // spineSize, spineSize, -fHeart-spineOffset, railSpacing);
    break;
  case genericflat:
  case smallflat:
  case bm:
  ...
  case doublespine:
    ...
    temp.radius.y = spineSize;
    options.append(temp);
    temp.offset.y = -fHeart - 0.45f * isInvert - spineOffset;
    options.append(temp);
    break;
  case rmc:
    temp.smooth = false;
    ...
    temp.offset.y = -fHeart + 0.043f * isInvert;
    ...
    temp.offset.y = -fHeart - 0.128f * isInvert;
    ...
    temp.offset.x = -railSpacing - 0.037f;
    temp.offset.y = -fHeart - 0.3f * isInvert;
  ...
  case gci:
    ...
    temp.offset.y = -fHeart + 0.0435f * isInvert;
    ...
    temp.offset.y = -fHeart - 0.0085f * isInvert;
    ...
    temp.offset.y = -fHeart - 0.1535f * isInvert;
...
#define BOX_WIDTH (0.05f)
  ...
  for (int i = 0; i < jSize; ++i) {
    ...
    // GENERIC
    case generic:
      P1 = curNode->vRelPos(-fHeart + 0.8 * railWidth * isInvert, -railSpacing,
                            -0.025f * isInvert);
      P2 = curNode->vRelPos(-fHeart + 0.8 * railWidth * isInvert, -railSpacing,
                            0.025f * isInvert);
      P3 = curNode->vRelPos(-fHeart, -railSpacing, -0.025f * isInvert);
      P4 = curNode->vRelPos(-fHeart, -railSpacing, 0.025f * isInvert);
      P5 = curNode->vRelPos(-fHeart - spineOffset + spineSize * 0.97 * isInvert,
                            0, -0.1f * isInvert);
      P6 = curNode->vRelPos(-fHeart - spineOffset + spineSize * 0.97 * isInvert,
                            0, 0.1f * isInvert);
      P7 = curNode->vRelPos(-fHeart - spineOffset - 0.8 * spineSize * isInvert,
                            -0.45 * spineSize, -0.1f * isInvert);
      P8 = curNode->vRelPos(-fHeart - spineOffset - 0.8 * spineSize * isInvert,
                            -0.45 * spineSize, 0.1f * isInvert);

      createBox(crossties, P1, P2, P3, P4, P5, P6, P7, P8);
      createShadowBox(crosstieshadows, P1, P2, P3, P4, P5, P6, P7, P8);
      P1 = curNode->vRelPos(-fHeart - spineOffset + spineSize * 0.97 * isInvert,
                            0, -0.1f * isInvert);
      P2 = curNode->vRelPos(-fHeart - spineOffset + spineSize * 0.97 * isInvert,
                            0, 0.1f * isInvert);
      P3 = curNode->vRelPos(-fHeart - spineOffset - 0.8 * spineSize * isInvert,
                            0.45 * spineSize, -0.1f * isInvert);
      P4 = curNode->vRelPos(-fHeart - spineOffset - 0.8 * spineSize * isInvert,
                            0.45 * spineSize, 0.1f * isInvert);
      P5 = curNode->vRelPos(-fHeart + 0.8 * railWidth * isInvert, railSpacing,
                            -0.025f * isInvert);
      P6 = curNode->vRelPos(-fHeart + 0.8 * railWidth * isInvert, railSpacing,
                            0.025f * isInvert);
      P7 = curNode->vRelPos(-fHeart, railSpacing, -0.025f * isInvert);
      P8 = curNode->vRelPos(-fHeart, railSpacing, 0.025f * isInvert);
    ...
    case genericflat:
      if (index % 6 == 0) {
        P1 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.8 : 1.0),
                              -railSpacing, -M);
        P2 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.8 : 1.0),
                              -railSpacing, M);
        P3 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 1.0 : 0.8),
                              -railSpacing, -M);
        P4 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 1.0 : 0.8),
                              -railSpacing, M);
        P5 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.8 : 1.0),
                              railSpacing, -M);
        P6 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.8 : 1.0),
                              railSpacing, M);
        P7 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 1.0 : 0.8),
                              railSpacing, -M);
        P8 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 1.0 : 0.8),
                              railSpacing, M);
      } else {
        P1 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.5 : 1),
                              -railSpacing, -0.03);
        P2 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.5 : 1),
                              -railSpacing, 0.03);
        P3 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 1 : 0.5),
                              -railSpacing, -0.03);
        P4 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 1 : 0.5),
                              -railSpacing, 0.03);
        P5 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.5 : 1),
                              railSpacing, -0.03);
        P6 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.5 : 1),
                              railSpacing, 0.03);
        P7 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 1 : 0.5),
                              railSpacing, -0.03);
        P8 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 1 : 0.5),
                              railSpacing, 0.03);
      }
      ...
      if (index) {
        P1 = lastNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.5 : 1),
                               -railSpacing + 0.13, 0.03);
        P2 = lastNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.5 : 1),
                               -railSpacing + 0.07, 0.03);
        P3 = lastNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 1 : 0.5),
                               -railSpacing + 0.13, 0.03);
        P4 = lastNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 1 : 0.5),
                               -railSpacing + 0.07, 0.03);
        P5 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.5 : 1),
                              railSpacing - 0.07, -0.03);
        P6 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.5 : 1),
    ...
    // STYLE
    case box:
      P1 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.6 : 0.8),
                            -railSpacing, -BOX_WIDTH);
      P2 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.6 : 0.8),
                            -railSpacing, BOX_WIDTH);
      P3 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.8 : 0.6),
                            -railSpacing, -BOX_WIDTH);
      P4 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.8 : 0.6),
                            -railSpacing, BOX_WIDTH);
      P5 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.6 : 0.8),
                            railSpacing, -BOX_WIDTH);
      P6 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.6 : 0.8),
                            railSpacing, BOX_WIDTH);
      P7 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.8 : 0.6),
                            railSpacing, -BOX_WIDTH);
      P8 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.8 : 0.6),
                            railSpacing, BOX_WIDTH);

      createBox(crossties, P1, P2, P3, P4, P5, P6, P7, P8);
      createShadowBox(crosstieshadows, P1, P2, P3, P4, P5, P6, P7, P8);

      P1 = curNode->vRelPos(-fHeart - spineOffset +
                                railWidth * (isInvert < 0 ? 0.6 : 0.8),
                            -railSpacing, -BOX_WIDTH);
      P2 = curNode->vRelPos(-fHeart - spineOffset +
                                railWidth * (isInvert < 0 ? 0.6 : 0.8),
                            -railSpacing, BOX_WIDTH);
      P3 = curNode->vRelPos(-fHeart - spineOffset -
                                railWidth * (isInvert < 0 ? 0.8 : 0.6),
                            -railSpacing, -BOX_WIDTH);
      P4 = curNode->vRelPos(-fHeart - spineOffset -
                                railWidth * (isInvert < 0 ? 0.8 : 0.6),
                            -railSpacing, BOX_WIDTH);
      P5 = curNode->vRelPos(-fHeart - spineOffset +
                                railWidth * (isInvert < 0 ? 0.6 : 0.8),
                            railSpacing, -BOX_WIDTH);
      P6 = curNode->vRelPos(-fHeart - spineOffset +
                                railWidth * (isInvert < 0 ? 0.6 : 0.8),
                            railSpacing, BOX_WIDTH);
      P7 = curNode->vRelPos(-fHeart - spineOffset -
                                railWidth * (isInvert < 0 ? 0.8 : 0.6),
                            railSpacing, -BOX_WIDTH);
      P8 = curNode->vRelPos(-fHeart - spineOffset -
                                railWidth * (isInvert < 0 ? 0.8 : 0.6),
                            railSpacing, BOX_WIDTH);
      ...
      P1 = curNode->vRelPos(
          -fHeart - isInvert * railWidth * 0.6,
          isInvert * (-railSpacing + BOX_INWARD + railWidth * 0.7), -BOX_WIDTH);
      P2 = curNode->vRelPos(
          -fHeart - isInvert * railWidth * 0.6,
          isInvert * (-railSpacing + BOX_INWARD + railWidth * 0.7), BOX_WIDTH);
      P3 = curNode->vRelPos(
      ...
      P4 = curNode->vRelPos(
          ...
          isInvert * (-railSpacing + BOX_INWARD - railWidth * 0.7), BOX_WIDTH);
      P5 = curNode->vRelPos(
          -fHeart - spineOffset + isInvert * railWidth * 0.6,
          isInvert * (-railSpacing + BOX_INWARD + railWidth * 0.7), -BOX_WIDTH);
      P6 = curNode->vRelPos(
          -fHeart - spineOffset + isInvert * railWidth * 0.6,
          isInvert * (-railSpacing + BOX_INWARD + railWidth * 0.7), BOX_WIDTH);
      ...
      P1 = curNode->vRelPos(-fHeart - isInvert * railWidth * 0.6,
                            -isInvert *
                                (-railSpacing + BOX_INWARD - railWidth * 0.7),
                            -BOX_WIDTH);
      P2 = curNode->vRelPos(
          -fHeart - isInvert * railWidth * 0.6,
          -isInvert * (-railSpacing + BOX_INWARD - railWidth * 0.7), BOX_WIDTH);
      P3 = curNode->vRelPos(-fHeart - isInvert * railWidth * 0.6,
                            -isInvert *
                                (-railSpacing + BOX_INWARD + railWidth * 0.7),
                            -BOX_WIDTH);
      P4 = curNode->vRelPos(
          -fHeart - isInvert * railWidth * 0.6,
          -isInvert * (-railSpacing + BOX_INWARD + railWidth * 0.7), BOX_WIDTH);
      P5 = curNode->vRelPos(-fHeart - spineOffset + isInvert * railWidth * 0.6,
                            -isInvert *
                                (-railSpacing + BOX_INWARD - railWidth * 0.7),
                            -BOX_WIDTH);
      P6 = curNode->vRelPos(
          -fHeart - spineOffset + isInvert * railWidth * 0.6,
          -isInvert * (-railSpacing + BOX_INWARD - railWidth * 0.7), BOX_WIDTH);
      P7 = curNode->vRelPos(-fHeart - spineOffset + isInvert * railWidth * 0.6,
                            -isInvert *
                                (-railSpacing + BOX_INWARD + railWidth * 0.7),
                            -BOX_WIDTH);
      P8 = curNode->vRelPos(
      ...
      if (index && index % 2) {
        P1 = curNode->vRelPos(-fHeart - isInvert * railWidth * 0.6,
                              ...
                              -BOX_WIDTH);
        P2 = curNode->vRelPos(-fHeart - isInvert * railWidth * 0.6,
                              isInvert *
                                  (-railSpacing + BOX_INWARD + railWidth * 0.5),
                              BOX_WIDTH);
        P3 = curNode->vRelPos(-fHeart - isInvert * railWidth * 0.6,
                              isInvert *
                                  (-railSpacing + BOX_INWARD - railWidth * 0.5),
                              -BOX_WIDTH);
        P4 = curNode->vRelPos(-fHeart - isInvert * railWidth * 0.6,
                              isInvert *
                                  (-railSpacing + BOX_INWARD - railWidth * 0.5),
                              BOX_WIDTH);
        P5 = lastNode->vRelPos(
            -fHeart - spineOffset + isInvert * railWidth * 0.6,
            isInvert * (-railSpacing + BOX_INWARD + railWidth * 0.5),
            -BOX_WIDTH);
        P6 = lastNode->vRelPos(
            -fHeart - spineOffset + isInvert * railWidth * 0.6,
            isInvert * (-railSpacing + BOX_INWARD + railWidth * 0.5),
        ...
        P1 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.6 : 0.4),
                              -railSpacing + BOX_DIAG1, -BOX_WIDTH);
        P2 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.6 : 0.4),
                              -railSpacing + BOX_DIAG0, -BOX_WIDTH);
        P3 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.4 : 0.6),
                              -railSpacing + BOX_DIAG1, -BOX_WIDTH);
        P4 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.4 : 0.6),
                              -railSpacing + BOX_DIAG0, -BOX_WIDTH);
        P5 = lastNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.6 : 0.4),
                               railSpacing - BOX_DIAG0, BOX_WIDTH);
        P6 = lastNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.6 : 0.4),
                               railSpacing - BOX_DIAG1, BOX_WIDTH);
        P7 = lastNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.4 : 0.6),
                               railSpacing - BOX_DIAG0, BOX_WIDTH);
        P8 = lastNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.4 : 0.6),
                               railSpacing - BOX_DIAG1, BOX_WIDTH);
        ...
        P1 = curNode->vRelPos(-fHeart - spineOffset -
                                  railWidth * (isInvert < 0 ? 0.6 : 0.4),
                              -railSpacing + BOX_DIAG1, -BOX_WIDTH);
        P2 = curNode->vRelPos(-fHeart - spineOffset -
                                  railWidth * (isInvert < 0 ? 0.6 : 0.4),
                              -railSpacing + BOX_DIAG0, -BOX_WIDTH);
        P3 = curNode->vRelPos(-fHeart - spineOffset +
                                  railWidth * (isInvert < 0 ? 0.4 : 0.6),
                              -railSpacing + BOX_DIAG1, -BOX_WIDTH);
        P4 = curNode->vRelPos(-fHeart - spineOffset +
                                  railWidth * (isInvert < 0 ? 0.4 : 0.6),
                              -railSpacing + BOX_DIAG0, -BOX_WIDTH);
        P5 = lastNode->vRelPos(-fHeart - spineOffset -
                                   railWidth * (isInvert < 0 ? 0.6 : 0.4),
                               railSpacing - BOX_DIAG0, BOX_WIDTH);
        P6 = lastNode->vRelPos(-fHeart - spineOffset -
                                   railWidth * (isInvert < 0 ? 0.6 : 0.4),
                               railSpacing - BOX_DIAG1, BOX_WIDTH);
        P7 = lastNode->vRelPos(-fHeart - spineOffset +
                                   railWidth * (isInvert < 0 ? 0.4 : 0.6),
                               railSpacing - BOX_DIAG0, BOX_WIDTH);
        P8 = lastNode->vRelPos(-fHeart - spineOffset +
                                   railWidth * (isInvert < 0 ? 0.4 : 0.6),
                               railSpacing - BOX_DIAG1, BOX_WIDTH);
        ...
        P1 = lastNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.4 : 0.6),
                               -railSpacing + BOX_DIAG1, BOX_WIDTH);
        P2 = lastNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.4 : 0.6),
                               -railSpacing + BOX_DIAG0, BOX_WIDTH);
        P3 = lastNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.6 : 0.4),
                               -railSpacing + BOX_DIAG1, BOX_WIDTH);
        P4 = lastNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.6 : 0.4),
                               -railSpacing + BOX_DIAG0, BOX_WIDTH);
        P5 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.4 : 0.6),
                              railSpacing - BOX_DIAG0, -BOX_WIDTH);
        P6 = curNode->vRelPos(-fHeart + railWidth * (isInvert > 0 ? 0.4 : 0.6),
                              railSpacing - BOX_DIAG1, -BOX_WIDTH);
        P7 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.6 : 0.4),
                              railSpacing - BOX_DIAG0, -BOX_WIDTH);
        P8 = curNode->vRelPos(-fHeart - railWidth * (isInvert > 0 ? 0.6 : 0.4),
                              railSpacing - BOX_DIAG1, -BOX_WIDTH);

        createBox(crossties, P1, P2, P3, P4, P5, P6, P7, P8);
        createShadowBox(crosstieshadows, P1, P2, P3, P4, P5, P6, P7, P8);

        P1 = lastNode->vRelPos(-fHeart - spineOffset +
                                   railWidth * (isInvert < 0 ? 0.4 : 0.6),
                               -railSpacing + BOX_DIAG1, BOX_WIDTH);
        P2 = lastNode->vRelPos(-fHeart - spineOffset +
                                   railWidth * (isInvert < 0 ? 0.4 : 0.6),
                               -railSpacing + BOX_DIAG0, BOX_WIDTH);
        P3 = lastNode->vRelPos(-fHeart - spineOffset -
                                   railWidth * (isInvert < 0 ? 0.6 : 0.4),
                               -railSpacing + BOX_DIAG1, BOX_WIDTH);
        P4 = lastNode->vRelPos(-fHeart - spineOffset -
                                   railWidth * (isInvert < 0 ? 0.6 : 0.4),
                               -railSpacing + BOX_DIAG0, BOX_WIDTH);
        P5 = curNode->vRelPos(-fHeart - spineOffset +
                                  railWidth * (isInvert < 0 ? 0.4 : 0.6),
    ...
    case doublespine:
      if (index % 2 == 0) {
        P1 = curNode->vRelPos(-fHeart + 0.75 * railWidth * isInvert,
                              -railSpacing, -0.12 * spineOffset);
        P2 = curNode->vRelPos(-fHeart + 0.75 * railWidth * isInvert,
                              -railSpacing, 0.12 * spineOffset);
        P3 = curNode->vRelPos(-fHeart - 0.25 * railWidth * isInvert,
                              -railSpacing, -0.12 * spineOffset);
        P4 = curNode->vRelPos(-fHeart - 0.25 * railWidth * isInvert,
                              -railSpacing, 0.12 * spineOffset);
        P5 = curNode->vRelPos(-fHeart + 0.75 * railWidth * isInvert,
                              railSpacing, -0.12 * spineOffset);
        P6 = curNode->vRelPos(-fHeart + 0.75 * railWidth * isInvert,
                              railSpacing, 0.12 * spineOffset);
        P7 = curNode->vRelPos(-fHeart - 0.25 * railWidth * isInvert,
                              railSpacing, -0.12 * spineOffset);
        P8 = curNode->vRelPos(-fHeart - 0.25 * railWidth * isInvert,
                              railSpacing, 0.12 * spineOffset);
        createBox(crossties, P1, P2, P3, P4, P5, P6, P7, P8);
        createShadowBox(crosstieshadows, P1, P2, P3, P4, P5, P6, P7, P8);

        P1 = curNode->vRelPos(-fHeart + 0.7 * railWidth * isInvert,
                              -0.80 * railSpacing, -0.10 * spineOffset);
        P2 = curNode->vRelPos(-fHeart + 0.7 * railWidth * isInvert,
                              -0.80 * railSpacing, 0.10 * spineOffset);
        P3 = curNode->vRelPos(-fHeart - 0.2 * railWidth * isInvert,
                              -0.80 * railSpacing, -0.10 * spineOffset);
        P4 = curNode->vRelPos(-fHeart - 0.2 * railWidth * isInvert,
                              -0.80 * railSpacing, 0.10 * spineOffset);
        P5 = curNode->vRelPos(-fHeart - spineOffset * 0.7, 0,
                              -0.20 * spineOffset);
        P6 = curNode->vRelPos(-fHeart - spineOffset * 0.7, 0,
                              0.20 * spineOffset);
        P7 =
            curNode->vRelPos(-fHeart - spineOffset - 0.9 * spineSize * isInvert,
                             0, -0.10 * spineOffset);
        P8 =
            curNode->vRelPos(-fHeart - spineOffset - 0.9 * spineSize * isInvert,
                             0, 0.10 * spineOffset);
        ...
        P1 = curNode->vRelPos(-fHeart - spineOffset * 0.7, 0,
                              -0.20 * spineOffset);
        P2 = curNode->vRelPos(-fHeart - spineOffset * 0.7, 0,
                              0.20 * spineOffset);
        P3 =
            curNode->vRelPos(-fHeart - spineOffset - 0.9 * spineSize * isInvert,
                             0, -0.10 * spineOffset);
        P4 =
            curNode->vRelPos(-fHeart - spineOffset - 0.9 * spineSize * isInvert,
                             0, 0.10 * spineOffset);
        P5 = curNode->vRelPos(-fHeart + 0.7 * railWidth * isInvert,
                              0.80 * railSpacing, -0.10 * spineOffset);
        P6 = curNode->vRelPos(-fHeart + 0.7 * railWidth * isInvert,
                              0.80 * railSpacing, 0.10 * spineOffset);
        P7 = curNode->vRelPos(-fHeart - 0.2 * railWidth * isInvert,
                              0.80 * railSpacing, -0.10 * spineOffset);
        P8 = curNode->vRelPos(-fHeart - 0.2 * railWidth * isInvert,
                              0.80 * railSpacing, 0.10 * spineOffset);
      ...
      if ((index + 3) % 4 == 0) {
        P1 = curNode->vRelPos(-fHeart - spineOffset, 0.65 * spineSize,
                              -0.65 * spineSize);
        P2 = curNode->vRelPos(-fHeart - spineOffset, 0.65 * spineSize,
                              0.65 * spineSize);
        P3 = curNode->vRelPos(-fHeart - spineOffset, -0.65 * spineSize,
                              -0.65 * spineSize);
        P4 = curNode->vRelPos(-fHeart - spineOffset, -0.65 * spineSize,
                              0.65 * spineSize);
        P5 = curNode->vRelPos(-fHeart - spineOffset - 0.4 * isInvert,
                              0.65 * spineSize, -0.65 * spineSize);
        P6 = curNode->vRelPos(-fHeart - spineOffset - 0.4 * isInvert,
                              0.65 * spineSize, 0.65 * spineSize);
        P7 = curNode->vRelPos(-fHeart - spineOffset - 0.4 * isInvert,
                              -0.65 * spineSize, -0.65 * spineSize);
        P8 = curNode->vRelPos(-fHeart - spineOffset - 0.4 * isInvert,
                              -0.65 * spineSize, 0.65 * spineSize);
    ...
    case mack:
      ...
      P1 = curNode->vRelPos(
          -fHeart - isInvert * railWidth * 0.8,
          isInvert * (-railSpacing + BOX_INWARD + railWidth / 2), -BOX_WIDTH);
      P2 = curNode->vRelPos(
          -fHeart - isInvert * railWidth * 0.8,
          isInvert * (-railSpacing + BOX_INWARD + railWidth / 2), BOX_WIDTH);
      P3 = curNode->vRelPos(
      ...
      P4 = curNode->vRelPos(
          -fHeart - isInvert * railWidth * 0.8,
          isInvert * (-railSpacing + BOX_INWARD - railWidth / 2), BOX_WIDTH);
      P5 = curNode->vRelPos(-fHeart - spineOffset, isInvert * railWidth / 2,
                            -BOX_WIDTH);
      P6 = curNode->vRelPos(-fHeart - spineOffset, isInvert * railWidth / 2,
                            BOX_WIDTH);
      P7 = curNode->vRelPos(-fHeart - spineOffset, -isInvert * railWidth / 2,
                            -BOX_WIDTH);
      P8 = curNode->vRelPos(-fHeart - spineOffset, -isInvert * railWidth / 2,
                            BOX_WIDTH);
      ...
      if (index && index % 2) {
        ...
        P4 = curNode->vRelPos(
            -fHeart - isInvert * railWidth * 0.8,
            isInvert * (-railSpacing + BOX_INWARD - railWidth / 2), BOX_WIDTH);
        P5 = lastNode->vRelPos(-fHeart - spineOffset, isInvert * railWidth / 2,
                               -BOX_WIDTH);
        P6 = lastNode->vRelPos(-fHeart - spineOffset, isInvert * railWidth / 2,
                               BOX_WIDTH);
        P7 = lastNode->vRelPos(-fHeart - spineOffset, -isInvert * railWidth / 2,
                               -BOX_WIDTH);
        P8 = lastNode->vRelPos(-fHeart - spineOffset, -isInvert * railWidth / 2,
                               BOX_WIDTH);
    ...
    case arrow:
      ...
      P2 = curNode->vRelPos(-fHeart - 0.7 * railWidth * isInvert,
                            -railSpacing - 0.5 * railWidth, +0.05f);
      P3 = curNode->vRelPos(-fHeart + 0.5 * railWidth * isInvert,
                            -railSpacing - 0.5 * railWidth, -0.05f);
      P4 = curNode->vRelPos(-fHeart + 0.5 * railWidth * isInvert,
                            -railSpacing - 0.5 * railWidth, +0.05f);
      P5 = curNode->vRelPos(-fHeart - 0.7 * railWidth * isInvert,
                            -1.2 * railSpacing, -0.05f);
      P6 = curNode->vRelPos(-fHeart - 0.7 * railWidth * isInvert,
                            -1.2 * railSpacing, +0.05f);
      P7 = curNode->vRelPos(-fHeart + 0.5 * railWidth * isInvert,
                            -1.2 * railSpacing, -0.05f);
      P8 = curNode->vRelPos(-fHeart + 0.5 * railWidth * isInvert,
                            -1.2 * railSpacing, +0.05f);
      ...
      if (isInvert > 0) {
        ...
        createBox(crossties, P5, P6, P7, P8, P1, P2, P3, P4);
        createShadowBox(crosstieshadows, P5, P6, P7, P8, P1, P2, P3, P4);
      }
      P1 = curNode->vRelPos(-fHeart + 0.5 * railWidth * isInvert,
                            railSpacing + 0.5 * railWidth, -0.05f);
      P2 = curNode->vRelPos(-fHeart + 0.5 * railWidth * isInvert,
                            railSpacing + 0.5 * railWidth, +0.05f);
      P3 = curNode->vRelPos(-fHeart - 0.7 * railWidth * isInvert,
                            railSpacing + 0.5 * railWidth, -0.05f);
      P4 = curNode->vRelPos(-fHeart - 0.7 * railWidth * isInvert,
                            railSpacing + 0.5 * railWidth, +0.05f);
      P5 = curNode->vRelPos(-fHeart + 0.5 * railWidth * isInvert,
                            1.2 * railSpacing, -0.05f);
      P6 = curNode->vRelPos(-fHeart + 0.5 * railWidth * isInvert,
                            1.2 * railSpacing, +0.05f);
      P7 = curNode->vRelPos(-fHeart - 0.7 * railWidth * isInvert,
                            1.2 * railSpacing, -0.05f);
      P8 = curNode->vRelPos(-fHeart - 0.7 * railWidth * isInvert,
                            1.2 * railSpacing, +0.05f);
      ...
      P4 = curNode->vRelPos(-fHeart - 0.7 * railWidth * isInvert,
                            -1.3 * railSpacing, +0.05f);
      P5 = curNode->vRelPos(-fHeart - 0.45 * isInvert, -1.2 * railSpacing,
                            -0.05f);
      P6 = curNode->vRelPos(-fHeart - 0.45 * isInvert, -1.2 * railSpacing,
                            +0.05f);
      P7 = curNode->vRelPos(-fHeart - 0.55 * isInvert, -1.3 * railSpacing,
                            -0.05f);
      P8 = curNode->vRelPos(-fHeart - 0.55 * isInvert, -1.3 * railSpacing,
                            +0.05f);
      ...
      P4 = curNode->vRelPos(-fHeart + 0.5 * railWidth * isInvert,
                            1.2 * railSpacing, +0.05f);
      P5 = curNode->vRelPos(-fHeart - 0.55 * isInvert, 1.3 * railSpacing,
                            -0.05f);
      P6 = curNode->vRelPos(-fHeart - 0.55 * isInvert, 1.3 * railSpacing,
                            +0.05f);
      P7 = curNode->vRelPos(-fHeart - 0.45 * isInvert, 1.2 * railSpacing,
                            -0.05f);
      P8 = curNode->vRelPos(-fHeart - 0.45 * isInvert, 1.2 * railSpacing,
                            +0.05f);
      ...
      P1 = curNode->vRelPos(-fHeart - 0.45 * isInvert, -1.3 * railSpacing,
                            -0.05f);
      P2 = curNode->vRelPos(-fHeart - 0.45 * isInvert, -1.3 * railSpacing,
                            +0.05f);
      P3 = curNode->vRelPos(-fHeart - 0.55 * isInvert, -1.3 * railSpacing,
                            -0.05f);
      P4 = curNode->vRelPos(-fHeart - 0.55 * isInvert, -1.3 * railSpacing,
                            +0.05f);
      P5 = curNode->vRelPos(-fHeart - 0.45 * isInvert, 1.3 * railSpacing,
                            -0.05f);
      P6 = curNode->vRelPos(-fHeart - 0.45 * isInvert, 1.3 * railSpacing,
                            +0.05f);
      P7 = curNode->vRelPos(-fHeart - 0.55 * isInvert, 1.3 * railSpacing,
                            -0.05f);
      P8 = curNode->vRelPos(-fHeart - 0.55 * isInvert, 1.3 * railSpacing,
      ...
      P1 = curNode->vRelPos(-fHeart - 0.48f * isInvert, -0.9 * railSpacing,
                            -0.07f);
      P2 = curNode->vRelPos(-fHeart - 0.48f * isInvert, -0.9 * railSpacing,
                            +0.07f);
      P3 = curNode->vRelPos(-fHeart - 0.58f * isInvert, -0.9 * railSpacing,
                            -0.07f);
      P4 = curNode->vRelPos(-fHeart - 0.58f * isInvert, -0.9 * railSpacing,
                            +0.07f);
      P5 = curNode->vRelPos(-fHeart - 0.48f * isInvert, -1.1 * spineSize,
                            -0.07f);
      P6 = curNode->vRelPos(-fHeart - 0.48f * isInvert, -1.1 * spineSize,
                            +0.07f);
      P7 = curNode->vRelPos(-fHeart - 0.9 * spineOffset -
                                0.5 * isInvert * (spineSize - 0.05),
                            -1.1 * spineSize, -0.07f);
      P8 = curNode->vRelPos(-fHeart - 0.9 * spineOffset -
                                0.5 * isInvert * (spineSize - 0.05),
                            -1.1 * spineSize, +0.07f);
      ...
      P4 = P8;
      P5 =
          curNode->vRelPos(-fHeart - 0.48f * isInvert, 1.1 * spineSize, -0.07f);
      P6 =
          curNode->vRelPos(-fHeart - 0.48f * isInvert, 1.1 * spineSize, +0.07f);
      P7 = curNode->vRelPos(-fHeart - 0.9 * spineOffset -
                                0.5 * isInvert * (spineSize - 0.05),
      ...
      P3 = P7;
      P4 = P8;
      P5 = curNode->vRelPos(-fHeart - 0.48f * isInvert, 0.9 * railSpacing,
                            -0.07f);
      P6 = curNode->vRelPos(-fHeart - 0.48f * isInvert, 0.9 * railSpacing,
                            +0.07f);
      P7 = curNode->vRelPos(-fHeart - 0.58f * isInvert, 0.9 * railSpacing,
                            -0.07f);
      P8 = curNode->vRelPos(-fHeart - 0.58f * isInvert, 0.9 * railSpacing,
                            +0.07f);
    ...
    case bm:
      ...
      if (index) {
        P1 = lastNode->vRelPos(
            -fHeart - spineOffset, // railWidth * (isInvert > 0 ? 0.5 : 1),
            spineWidth, 0);
        P2 = lastNode->vRelPos(
            -fHeart - spineOffset, // railWidth * (isInvert > 0 ? 0.5 : 1),
            -spineWidth, 0);
        P3 = lastNode->vRelPos(
            -fHeart - spineBottom, // railWidth * (isInvert > 0 ? 1 : 0.5),
            spineWidth, 0);
        P4 = lastNode->vRelPos(
            -fHeart - spineBottom, // railWidth * (isInvert > 0 ? 1 : 0.5),
            -spineWidth, 0);
      ...
      if (index % 6 == 0) {
        loadObj(crossties, crosstieshadows,
                ":/crossties/bm_classic_connector.obj", curNode, fHeart, scale);
      } else {
        loadObj(crossties, crosstieshadows, ":/crossties/bm_classic.obj",
                curNode, fHeart, scale);
      }
      break;
    case bmclassic8dive:
      ...
      //   curNode, fHeart, scale);
      // }
      loadObj(crossties, crosstieshadows, ":/crossties/bm_classic8Dive.obj",
              curNode, fHeart, scale);
      break;
    case newbm:
      ...
      if (index % 6 == 0) {
        loadObj(crossties, crosstieshadows,
                ":/crossties/bm_modern_connector.obj", curNode, fHeart, scale);
      } else {
        loadObj(crossties, crosstieshadows, ":/crossties/bm_modern.obj",
                curNode, fHeart, scale);
      }
      break;
    case rmc:
      P1 = curNode->vRelPos(-fHeart - 0.303f * isInvert, -railSpacing - 0.176f,
                            -0.051f * isInvert);
      P2 = curNode->vRelPos(-fHeart - 0.303f * isInvert, -railSpacing - 0.176f,
                            0.051f * isInvert);
      P3 = curNode->vRelPos(-fHeart - 0.315f * isInvert, -railSpacing - 0.176f,
                            -0.051f * isInvert);
      P4 = curNode->vRelPos(-fHeart - 0.315f * isInvert, -railSpacing - 0.176f,
                            0.051f * isInvert);
      P5 = curNode->vRelPos(-fHeart - 0.303f * isInvert, railSpacing + 0.176f,
                            -0.051f * isInvert);
      P6 = curNode->vRelPos(-fHeart - 0.303f * isInvert, railSpacing + 0.176f,
                            0.051f * isInvert);
      P7 = curNode->vRelPos(-fHeart - 0.315f * isInvert, railSpacing + 0.176f,
                            -0.051f * isInvert);
      P8 = curNode->vRelPos(-fHeart - 0.315f * isInvert, railSpacing + 0.176f,
                            0.051f * isInvert);
      ...
      P1 = curNode->vRelPos(-fHeart - 0.435f * isInvert, -railSpacing - 0.176f,
                            -0.051f * isInvert);
      P2 = curNode->vRelPos(-fHeart - 0.435f * isInvert, -railSpacing - 0.176f,
                            -0.039f * isInvert);
      P3 = curNode->vRelPos(-fHeart - 0.435f * isInvert, railSpacing + 0.176f,
                            -0.051f * isInvert);
      P4 = curNode->vRelPos(-fHeart - 0.435f * isInvert, railSpacing + 0.176f,
                            -0.039f * isInvert);
      P5 = curNode->vRelPos(-fHeart - 0.315f * isInvert, -railSpacing - 0.176f,
                            -0.051f * isInvert);
      P6 = curNode->vRelPos(-fHeart - 0.315f * isInvert, -railSpacing - 0.176f,
                            -0.039f * isInvert);
      P7 = curNode->vRelPos(-fHeart - 0.315f * isInvert, railSpacing + 0.176f,
                            -0.051f * isInvert);
      P8 = curNode->vRelPos(-fHeart - 0.315f * isInvert, railSpacing + 0.176f,
      ...
      break;
    case gci:
      P1 = curNode->vRelPos(-fHeart - 0.2685f * isInvert, -railSpacing - 0.6f,
                            -0.05f * isInvert);
      P2 = curNode->vRelPos(-fHeart - 0.2685f * isInvert, -railSpacing - 0.6f,
                            0.05f * isInvert);
      P3 = curNode->vRelPos(-fHeart - 0.3685f * isInvert, -railSpacing - 0.6f,
                            -0.05f * isInvert);
      P4 = curNode->vRelPos(-fHeart - 0.3685f * isInvert, -railSpacing - 0.6f,
                            0.05f * isInvert);
      P5 = curNode->vRelPos(-fHeart - 0.2685f * isInvert, railSpacing + 0.6f,
                            -0.05f * isInvert);
      P6 = curNode->vRelPos(-fHeart - 0.2685f * isInvert, railSpacing + 0.6f,
                            0.05f * isInvert);
      P7 = curNode->vRelPos(-fHeart - 0.3685f * isInvert, railSpacing + 0.6f,
                            -0.05f * isInvert);
      P8 = curNode->vRelPos(-fHeart - 0.3685f * isInvert, railSpacing + 0.6f,
                            0.05f * isInvert);
    ...
    case vekoma: // y, x, z
      // beam
      loadObj(crossties, crosstieshadows, ":/crossties/vekoma_mk1101_mid.obj",
              curNode, fHeart, scale);
