...
myFramebuffer::myFramebuffer(int _width, int _height, GLuint _format,
...
GLuint myFramebuffer::getHandle() { return handle; }

GLuint myFramebuffer::getTexture() { return target->getId(); }

void myFramebuffer::bind() { glBindFramebuffer(GL_FRAMEBUFFER, handle); }
...
void myFramebuffer::unbind() { glBindFramebuffer(GL_FRAMEBUFFER, 0); }

void myFramebuffer::printFBStatus() {
  GLuint status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
  switch (status) {
  case GL_FRAMEBUFFER_UNDEFINED:
    qWarning("Target is the default framebuffer, but the default framebuffer "
             "does not exist.");
    break;
  ...
  case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
    qWarning("The value of GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE is GL_NONE "
             "for any color attachment point(s) named by GL_DRAW_BUFFERi.");
    break;
  case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
    qWarning("GL_READ_BUFFER is not GL_NONE and the value of "
             "GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE is GL_NONE for the color "
             "attachment point named by GL_READ_BUFFER.");
    break;
  case GL_FRAMEBUFFER_UNSUPPORTED:
    qWarning("The combination of internal formats of the attached images "
  ...
  case GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS:
    qWarning("is returned if any framebuffer attachment is layered, and any "
