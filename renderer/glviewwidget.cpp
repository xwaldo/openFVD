...
glViewWidget::glViewWidget(QWidget *parent) : QGLWidget(parent) {
  ...
  stlLoaded.resize(nStlFiles);
  stlShader.resize(nStlFiles);
  stlCountVertices.resize(nStlFiles);
  stlTriangles.resize(nStlFiles);
  stlVertices.resize(nStlFiles);
  stlCountTriangles.resize(nStlFiles);
  stlMesh.resize(nStlFiles);
  stlColor.resize(nStlFiles);

  for (int i = 0; i < nStlFiles; ++i) {
    stlLoaded[i] = false;
  }
}
...
glViewWidget::~glViewWidget() {
  if (!legacyMode) {
    for (int i = 0; i < stlShader.size(); ++i) {
      delete stlShader[0];
      delete stlShader[1];
    }
...
void glViewWidget::drawFloor() {
  ...
  float sizeTex;
  if (gloParent->mOptions->measures == 2) {
    sizeTex = grdTexSize * 0.3048;
  } else {
    sizeTex = grdTexSize;
  }
  floorShader->useUniform("grdTexSize", sizeTex);

  if (shadowMode == 0)
    floorShader->useUniform("shadowTex", simpleShadowFb->getTexture());
  else if (shadowMode > 0)
    floorShader->useUniform("shadowTex", shadowVolumeFb->getTexture());
...
void glViewWidget::drawSky() {
  ...
  bottomRight =
      -glm::vec4(glm::normalize(glm::vec3(bottomRight) - cameraPos), 0);
...
void glViewWidget::drawStl(myShader *stlShader, int meshIndex) {
  glm::mat4 anchorBase = glm::translate(glm::vec3(0.f, 0.f, 0.f)) *
                         glm::rotate(TO_RAD(90.f), glm::vec3(0.f, 1.f, 0.f));

  stlShader->bind();
  stlShader->useUniform("projectionMatrix", &ProjectionMatrix);
  stlShader->useUniform("modelMatrix", &ModelMatrix);
  stlShader->useUniform("anchorBase", &anchorBase);
  stlShader->useUniform("eyePos", &cameraPos);

  stlShader->useUniform("metalTex", metalTexture->getId());
  stlShader->useUniform("skyTex", skyTexture->getId());
  stlShader->useUniform("occlusionTex", occlusionFb->getTexture());

  if (shadowMode == 0)
    stlShader->useUniform("shadowTex", simpleShadowFb->getTexture());
  else if (shadowMode > 0)
    stlShader->useUniform("shadowTex", shadowVolumeFb->getTexture());

  stlShader->useUniform("lightDir", &lightDir);

  stlShader->useUniform("wire", (GLuint)1);
  stlShader->useUniform("edgeWidth", (float)0.003f);

  stlShader->useUniform("solidColor", stlColor[meshIndex].red() / 255.f,
                        stlColor[meshIndex].green() / 255.f,
                        stlColor[meshIndex].blue() / 255.f);

  glBindVertexArray(stlMesh[meshIndex].object);
  glDrawArrays(GL_TRIANGLES, 0, stlCountTriangles[meshIndex]);
  //  // VERY COSTLY (ONLY FOR DEBUG)
...
void glViewWidget::drawTrack(trackHandler *_track, bool toNormalMap) {
  ...
  glm::mat4 anchorBase =
      glm::translate(myTrack->startPos) *
      glm::rotate(TO_RAD(myTrack->startYaw - 90.f), glm::vec3(0.f, 1.f, 0.f));
  ...
  if (shadowMode == 0)
    shader->useUniform("shadowTex", simpleShadowFb->getTexture());
  else if (shadowMode > 0)
    shader->useUniform("shadowTex", shadowVolumeFb->getTexture());

  QColor *tempColor = _track->trackColors;
  shader->useUniform("defaultColor", tempColor[0].red() / 255.f,
                     tempColor[0].green() / 255.f, tempColor[0].blue() / 255.f);
  shader->useUniform("sectionColor", tempColor[1].red() / 255.f,
                     tempColor[1].green() / 255.f, tempColor[1].blue() / 255.f);
  shader->useUniform("transitionColor", tempColor[2].red() / 255.f,
                     tempColor[2].green() / 255.f, tempColor[2].blue() / 255.f);
  ...
  if (myTrack->drawHeartline != 2 && myTrack->lSections.size() != 0) {
    ...
    for (int i = 0; i < mesh->pipeBorders.size() - 1; ++i) {
      // glDrawArrays(GL_TRIANGLE_STRIP, i*mesh->trackVertexSize,
      // mesh->trackVertexSize);
      if (mesh->isWireframe) {
        glDrawElements(
            GL_LINE_STRIP, mesh->pipeBorders[i + 1] - mesh->pipeBorders[i],
            GL_UNSIGNED_INT, (GLvoid *)(sizeof(GLuint) * mesh->pipeBorders[i]));
      } else {
        glDrawElements(
            GL_TRIANGLE_STRIP, mesh->pipeBorders[i + 1] - mesh->pipeBorders[i],
            GL_UNSIGNED_INT, (GLvoid *)(sizeof(GLuint) * mesh->pipeBorders[i]));
      }
      // glDrawArrays(GL_POINTS, i*mesh->trackVertexSize,
      // mesh->trackVertexSize);
    }
...
void glViewWidget::drawShadowVolumes() {
  ...
  anchorBase = glm::translate(glm::vec3(0.f, 0.f, 0.f)) *
               glm::rotate(TO_RAD(90.f), glm::vec3(0.f, 1.f, 0.f));

  shadowVolumeShader->useUniform("anchorBase", &anchorBase);

  for (int i = 0; i < stlLoaded.size(); ++i) {
    if (stlLoaded[i]) {
      glBindVertexArray(stlMesh[i].object);
      glDrawArrays(GL_TRIANGLES, 0, stlCountTriangles[i]);
    }
  }
  ...
  for (int i = 0; i < trackList.size(); ++i) {
    if (trackList[i]->trackData->drawTrack) {
      ...
      anchorBase = glm::translate(myTrack->startPos) *
                   glm::rotate(TO_RAD(myTrack->startYaw - 90.f),
        ...
        for (int i = 0; i < mesh->pipeBorders.size() - 1; ++i) {
          if (mesh->isWireframe) {
            glDrawElements(GL_LINE_STRIP,
                           mesh->pipeBorders[i + 1] - mesh->pipeBorders[i],
                           GL_UNSIGNED_INT,
                           (GLvoid *)(sizeof(GLuint) * mesh->pipeBorders[i]));
          } else {
  ...
  for (int i = 0; i < trackList.size(); ++i) {
    if (trackList[i]->trackData->drawTrack &&
        trackList[i]->trackData->drawHeartline != 2 &&
      ...
      glDrawElements(GL_TRIANGLES, mesh->shadowIndices.size(), GL_UNSIGNED_INT,
                     (GLvoid *)0);
...
void glViewWidget::drawOcclusion() {
  ...
  occlusionShader->useUniform("BR", bottomRight.x, bottomRight.y,
                              bottomRight.z);
...
void glViewWidget::paintGL() {
  if (initialized != 2)
    return;
  if (gloParent->mGraphWidget)
    gloParent->mGraphWidget->setBezPoints();
  if (!paintMode)
    return;
  ...
  if (!legacyMode) {
    ...
    if (!riftMode /* || !gloParent->isFullScreen()*/) {
      ...
      for (int i = 0; i < trackList.size(); ++i) {
        if (trackList[i]->trackData->drawTrack) {
          ...
          if (shadowMode == 0 || trackList[i]->mMesh->isWireframe ||
              trackList[i]->trackData->drawHeartline == 2) {
    ...
    } else // rift mode
    {
      ...
      for (int p = 0; p < 2; ++p) {
        ...
        glViewport(p * 2 * viewPortWidth / 2, 0, 2 * viewPortWidth / 2,
                   2 * viewPortHeight);
    ...
    if (!povMode) {
      gluLookAt(cameraPos.x, cameraPos.y, cameraPos.z,
                cameraPos.x + freeFlyDir.x, cameraPos.y + freeFlyDir.y,
                cameraPos.z + freeFlyDir.z, 0.0, 1.0, 0.0);
    } else {
      glm::mat4 anchorBase =
          glm::translate(gloParent->curTrack()->startPos) *
          glm::rotate(TO_RAD(gloParent->curTrack()->startYaw - 90.f),
                      glm::vec3(0.f, 1.f, 0.f));
      glm::vec3 vNormal =
          glm::vec3(anchorBase * glm::vec4(-povNode->vNorm, 0.f));
      glm::vec3 vLookAt = glm::vec3(
          anchorBase *
          glm::vec4(povNode->vPos +
                        povNode->vDirHeart(gloParent->curTrack()->fHeart),
                    1.f));
      glm::vec3 vPosition =
          glm::vec3(anchorBase * glm::vec4(povNode->vPos, 1.f));
      gluLookAt(vPosition.x, vPosition.y, vPosition.z, vLookAt.x, vLookAt.y,
                vLookAt.z, vNormal.x, vNormal.y, vNormal.z);
    }
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    gluPerspective(fov * 0.5, this->width() / ((double)this->height()), DNEAR,
                   DFAR);
...
void glViewWidget::setGroundTextureSize(float _grdTexSize) {
  grdTexSize = _grdTexSize;
}
...
void glViewWidget::setBackgroundColor(QColor _background) {
  // if(legacyMode)
  {
    ...
    glClearColor(_background.red() / 255.f, _background.green() / 255.f,
                 _background.blue() / 255.f, 1.f);
  }
  /*else
  {
      clearColor = _background;
...
void glViewWidget::setStlColor(QColor _color, int meshIndex) {
  stlColor[meshIndex] = _color;
}

void glViewWidget::clearStlMesh() {
  for (int i = 0; i < stlLoaded.size(); ++i) {
    stlLoaded[i] = false;
    stlTriangles[i].clear();
    stlVertices[i].clear();
  }
}
...
void glViewWidget::initializeGL() {
  ...
  qDebug("Samples: %d %d", this->format().sampleBuffers(),
         this->format().samples());
  ...
  setBackgroundColor(gloParent->mOptions->backgroundColor);
  setStlColor(QColor(140, 140, 140, 1), 0);
  setStlColor(QColor(233, 250, 0, 1), 1);
  ...
  if (riftMode) {
    ...
    Ptr<HMDDevice> pHMD =
        *pManager->EnumerateDevices<HMDDevice>(true).CreateDevice();
    HMDInfo info;
    sensor = NULL;
    sensFusion = NULL;
...
void glViewWidget::resizeGL(int w, int h) {
  ...
  if (riftMode /* && gloParent->isFullScreen()*/) {
    ...
    if (simpleShadowFb)
      simpleShadowFb->resize(w / 2, h);
    if (shadowVolumeFb)
      shadowVolumeFb->resize(w / 2, h);
    if (normalMapFb)
      normalMapFb->resize(w / 2, h);
    if (occlusionFb)
      occlusionFb->resize(w / 2, h);
  } else {
    glViewport(0, 0, w, h);
    if (simpleShadowFb)
      simpleShadowFb->resize(w, h);
    if (shadowVolumeFb)
      shadowVolumeFb->resize(w, h);
    if (normalMapFb)
      normalMapFb->resize(w, h);
    if (occlusionFb)
      occlusionFb->resize(w, h);
  }
  if (preDistortionFb)
    preDistortionFb->resize(2 * w, 2 * h);
}
...
void glViewWidget::mousePressEvent(QMouseEvent *event) {
  ...
  if (moveMode) {
    ...
    if (this->mapFromGlobal(mousePos).x() < 80) {
      mousePos.setX(
          this->mapToGlobal(QPoint(80, this->mapFromGlobal(mousePos).y())).x());
    } else if (this->mapFromGlobal(mousePos).x() > this->width() - 80) {
      mousePos.setX(this->mapToGlobal(QPoint(this->width() - 80,
                                             this->mapFromGlobal(mousePos).y()))
                        .x());
    }
    if (this->mapFromGlobal(mousePos).y() < 80) {
      mousePos.setY(
          this->mapToGlobal(QPoint(this->mapFromGlobal(mousePos).x(), 80)).y());
    } else if (this->mapFromGlobal(mousePos).y() > this->height() - 80) {
      mousePos.setY(this->mapToGlobal(QPoint(this->mapFromGlobal(mousePos).x(),
                                             this->height() - 80))
                        .y());
    }
    ...
    this->grabKeyboard();
    hasChanged = true;
    // this->cursor().setPos(this->mapToGlobal(QPoint(this->width()/2.0,
    // this->height()/2.0)));
  } else {
...
void glViewWidget::initFloorMesh() {
  ...
  float floor[31 * 6] = {
      -a,  0.f, +a,  0.f, 0.5f, 0.f, // STILL TO DO
      -a,  0.f, +b,  0.f, 0.5f, 0.f, -b, 0.f, +a, 0.f, 0.5f, 0.f,
      -b,  0.f, +b,  0.f, 0.5f, 0.f, +b, 0.f, +a, 0.f, 0.5f, 0.f,
      +b,  0.f, +b,  0.f, 0.5f, 0.f, +a, 0.f, +a, 0.f, 0.5f, 0.f,
      +a,  0.f, +b,  0.f, 0.5f, 0.f, +a, 0.f, +b, 0.f, 0.5f, 0.f,
      +b,  0.f, +b,  0.f, 0.5f, 0.f,

      +a,  0.f, -b,  0.f, 0.5f, 0.f, +b, 0.f, -b, 0.f, 0.5f, 0.f,
      +a,  0.f, -a,  0.f, 0.5f, 0.f, +b, 0.f, -a, 0.f, 0.5f, 0.f,
      +b,  0.f, -b,  0.f, 0.5f, 0.f,

      -b,  0.f, -a,  0.f, 0.5f, 0.f, -b, 0.f, -b, 0.f, 0.5f, 0.f,
      -a,  0.f, -a,  0.f, 0.5f, 0.f, -a, 0.f, -b, 0.f, 0.5f, 0.f,
      -a,  0.f, -b,  0.f, 0.5f, 0.f, -b, 0.f, -b, 0.f, 0.5f, 0.f,

      -a,  0.f, +b,  0.f, 0.5f, 0.f, -b, 0.f, +b, 0.f, 0.5f, 0.f,
      -b,  0.f, +b,  0.f, 0.5f, 0.f, -b, 0.f, -b, 0.f, 0.5f, 0.f,
      0.f, 0.f, 0.f, 0.f, 0.5f, 0.f, +b, 0.f, -b, 0.f, 0.5f, 0.f,
      0.f, 0.f, 0.f, 0.f, 0.5f, 0.f, +b, 0.f, +b, 0.f, 0.5f, 0.f,
      0.f, 0.f, 0.f, 0.f, 0.5f, 0.f, -b, 0.f, +b, 0.f, 0.5f, 0.f};
  ...
  float sky[12] = {1.f,  1.f, 0.9999f, 1.f,  -1.f, 0.9999f,
                   -1.f, 1.f, 0.9999f, -1.f, -1.f, 0.9999f};
...
void glViewWidget::loadStlMesh(QString fileName, int meshIndex) {
  // if we don't clear, previously loaded .stl objects will remain
  stlTriangles[meshIndex].clear();
  if (!readStl(fileName, stlTriangles[meshIndex])) {
    qWarning() << "Failed to read STL file.";
    return;
  }
  stlLoaded[meshIndex] = initStlMesh(meshIndex);
}

bool glViewWidget::initStlMesh(int meshIndex) {
  stlVertices[meshIndex].clear();
  // stlVertices[meshIndex] = extractVertices(stlTriangles[meshIndex]);
  stlVertices[meshIndex] = extractVerticesNormal(stlTriangles[meshIndex]);
  stlCountVertices[meshIndex] = 3 * stlVertices[meshIndex].size();
  stlCountTriangles[meshIndex] = 3 * stlTriangles[meshIndex].size();

  glGenVertexArrays(1, &stlMesh[meshIndex].object);
  glGenBuffers(1, &stlMesh[meshIndex].buffer);

  glBindVertexArray(stlMesh[meshIndex].object);

  glBindBuffer(GL_ARRAY_BUFFER, stlMesh[meshIndex].buffer);
  glBufferData(GL_ARRAY_BUFFER, stlCountVertices[meshIndex] * sizeof(GL_FLOAT),
               stlVertices[meshIndex].constData(), GL_STATIC_DRAW);
...
#define RASTER_SIZE (600)
  ...
  for (int i = 0; i < RASTER_SIZE; ++i) {
    for (int j = 0; j < RASTER_SIZE; ++j) {
      // float col = std::min(fabs(i-RASTER_SIZE/2.f),
      // fabs(j-RASTER_SIZE/2.f))/(RASTER_SIZE/2.f);
      double major =
          std::min(fabs(i - RASTER_SIZE / 2.f), fabs(j - RASTER_SIZE / 2.f)) /
          (RASTER_SIZE / 2.f);
      double minor = std::min(
          (double)std::min(fabs(((i + (RASTER_SIZE / 20)) % (RASTER_SIZE / 10) -
                                 (RASTER_SIZE / 20.f))),
                           fabs(((j + (RASTER_SIZE / 20)) % (RASTER_SIZE / 10) -
                                 (RASTER_SIZE / 20.f)))),
          1.);
      major = 1 - major;
      minor = 1 - minor;
      ...
      raster->setPixel(
          i, j, 0xff000000 + ((int)col << 16) + ((int)col << 8) + (int)col);
    }
  }
  rasterTexture = new myTexture(*raster, 1);
  loadGroundTexture(":/background.png");
  skyTexture =
      new myTexture(":/sky/negx.jpg", ":/sky/negy.jpg", ":/sky/negz.jpg",
                    ":/sky/posx.jpg", ":/sky/posy.jpg", ":/sky/posz.jpg");
...
void glViewWidget::initShaders() {
  ...
  for (int i = 0; i < stlLoaded.size(); ++i) {
    stlShader[i] = new myShader(":/shaders/stl.vert", ":/shaders/stl.frag");
    stlShader[i]->useAttribute(0, "aPosition");
    stlShader[i]->useAttribute(1, "aNormal");
    stlShader[i]->useAttribute(2, "aUv"); // TODO
    stlShader[i]->linkProgram();
  }
  ...
  simpleShadowFb =
      new myFramebuffer(viewPortWidth, viewPortHeight, GL_RED, GL_RED);

  simpleSMShader =
      new myShader(":/shaders/simpleSM.vert", ":/shaders/simpleSM.frag");
  simpleSMShader->useAttribute(0, "aPosition");
  simpleSMShader->setOutput(0, "visibility");
  simpleSMShader->linkProgram();

  shadowVolumeFb =
      new myFramebuffer(viewPortWidth, viewPortHeight, GL_RED, GL_RED, true);
  shadowVolumeFb->setClearColor(0, 0, 0);

  shadowVolumeShader = new myShader(":/shaders/shadowVolume.vert",
  ...
  normalMapFb = new myFramebuffer(viewPortWidth, viewPortHeight, GL_RGBA16F,
                                  GL_RGBA, true);
  normalMapFb->setClearColor(0, 0, 0);

  normalMapShader =
      new myShader(":/shaders/normals.vert", ":/shaders/normals.frag");
  ...
  preDistortionFb =
      new myFramebuffer(viewPortWidth, viewPortHeight, GL_RGBA, GL_RGBA, true);
  preDistortionFb->setClearColor(0, 0, 0);

  occlusionShader =
      new myShader(":/shaders/occlusion.vert", ":/shaders/occlusion.frag");
...
void glViewWidget::moveCamera() {
  if (povMode) {
    povPos +=
        (int)(F_HZ * renderTime) * cameraBoost * (cameraMov.x - cameraMov.z);
    if (gloParent->curTrack()) {
      if (povPos < 0) {
  ...
  if (moveMode) {
    ...
    if (!povMode) {
      freeFlyDir = glm::angleAxis(TO_RAD(rotateX), sign * glm::vec3(0, 1, 0)) *
                   freeFlyDir;
      freeFlySide = glm::angleAxis(TO_RAD(rotateX), sign * glm::vec3(0, 1, 0)) *
                    freeFlySide;
      freeFlyDir = glm::angleAxis(TO_RAD(rotateY), freeFlySide) * freeFlyDir;
      // freeFlyDir = glm::vec3(glm::rotate(, glm::cross(freeFlyDir,
      // glm::vec3(0.f, 1.f, 0.f))) * glm::vec4(freeFlyDir, 0.f));
    }
    this->cursor().setPos(mousePos);
  }
  ...
  if (!povMode) {
    freeFlyPos +=
        0.3f * cameraBoost * (cameraMov.x - cameraMov.z) * freeFlyDir -
        0.3f * cameraBoost * (cameraMov.y - cameraMov.w) *
            glm::normalize(glm::cross(freeFlyDir, glm::vec3(0.f, 1.f, 0.f)));
    freeFlyPos +=
        0.01f * cameraJump * cameraBoost *
        glm::cross(freeFlyDir, glm::normalize(glm::cross(
                                   freeFlyDir, glm::vec3(0.f, 1.f, 0.f))));
  }
#ifdef USE_OVR
  Quatf bla;
  if (sensFusion) {
  ...
  bla.GetEulerAngles<Axis_Y, Axis_X, Axis_Z>(&headPos.z, &headPos.y,
                                             &headPos.x);
#endif
  cameraJump = 0;
}
...
void glViewWidget::buildMatrices(float offset) {
  ...
  if (riftMode /* && gloParent->isFullScreen()*/) {
    // Matrix4f bla =
    // Matrix4f::PerspectiveRH(2*atan(vScreenSize/(2*fEyeToScreen)),
    // viewPortWidth/(2.f*viewPortHeight), FNEAR, FFAR);
    float fov = OCULUS_SCALE * vScreenSize / (2 * fEyeToScreen);
    float aspect = viewPortWidth / (2.f * viewPortHeight);
    ProjectionMatrix = glm::frustum(-aspect * fov * FNEAR, aspect * fov * FNEAR,
                                    -fov * FNEAR, fov * FNEAR, FNEAR, FFAR);

    float h =
        (offset / fabs(offset)) * (hScreenSize - lensSep * 2.f) / hScreenSize;
    if (h != h)
      h = 0.f;
    ProjectionMatrix =
        glm::translate(glm::vec3(h, 0.f, 0.f)) * ProjectionMatrix;
  } else {
    ProjectionMatrix =
        glm::frustum(-side + scew, side + scew,
                     -(float)viewPortHeight / viewPortWidth * side,
                     (float)viewPortHeight / viewPortWidth * side, FNEAR, FFAR);
  }
  if (povMode) {
    anchorBase = glm::translate(gloParent->curTrack()->startPos) *
                 glm::rotate(TO_RAD(gloParent->curTrack()->startYaw - 90.f),
                             glm::vec3(0.f, 1.f, 0.f));
    glm::vec3 pos = povNode->vRelPos(gloParent->curTrack()->povPos.y,
                                     gloParent->curTrack()->povPos.x);
    ...
    direction = glm::angleAxis((float)(-headPos.z), down) *
                glm::angleAxis((float)(headPos.y * 180.f / F_PI), side) *
                direction;
    ...
    ModelMatrix =
        glm::lookAt(glm::vec3(anchorBase * glm::vec4(pos, 1.f)),
                    glm::vec3(anchorBase * glm::vec4(pos + direction, 1.f)),
                    -glm::vec3(anchorBase * glm::vec4(down, 0.f)));
    cameraPos = glm::vec3(anchorBase * glm::vec4(pos, 1.f));
    cameraDir = glm::vec3(
        anchorBase *
        glm::vec4(povNode->vDirHeart(gloParent->curTrack()->fHeart), 0.f));
  } else {
    cameraPos = freeFlyPos;
    glm::vec3 pos = cameraPos + offset * freeFlySide;

    ModelMatrix =
        glm::lookAt(pos, pos + freeFlyDir, glm::cross(freeFlySide, freeFlyDir));
...
void glViewWidget::legacyDrawFloor() {
  ...
  for (int i = -210; i <= 210; i += 10) {
    glBegin(GL_LINES);
    glVertex3f((float)i, 0.0, -220);
    glVertex3f((float)i, 0.0, 220);
    glEnd();
    glBegin(GL_LINES);
    glVertex3f(-220, 0.0, (float)i);
    glVertex3f(220, 0.0, (float)i);
    glEnd();
  }
}
...
void glViewWidget::legacyDrawTrack(trackHandler *_track) {
  if (_track->trackData->drawHeartline == 2)
    return;
  ...
  for (int i = 0; i < myTrack->lSections.size(); i++) {
    ...
    if (curSection == myTrack->activeSection) {
      glColor3f(_track->trackColors[1].red() / 255.f,
                _track->trackColors[1].green() / 255.f,
                _track->trackColors[1].blue() / 255.f);
    } else {
      glColor3f(_track->trackColors[0].red() / 255.f,
                _track->trackColors[0].green() / 255.f,
                _track->trackColors[0].blue() / 255.f);
    }
    ...
    for (int j = 0; j < curSection->lNodes.size(); ++j) {
      ...
      if (distFromLastNode >= 1.f || j == curSection->lNodes.size() - 1) {
        ...
        curPos =
            anchorBase *
            glm::vec4(curSection->lNodes[j].vPosHeart(myTrack->fHeart + fSpine),
                      1.f);
    ...
    curPos =
        anchorBase *
        glm::vec4(curSection->lNodes[end].vPosHeart(myTrack->fHeart + fSpine),
                  1.f);
    glVertex3f(curPos.x, curPos.y, curPos.z);
    glEnd();
  }
  ...
  for (int i = 0; i < myTrack->lSections.size(); i++) {
    ...
    for (int j = 0; j < curSection->lNodes.size(); ++j) {
      ...
      if (distFromLastNode >= 1.f || j == curSection->lNodes.size() - 1) {
        ...
        curPos =
            anchorBase *
            glm::vec4(curSection->lNodes[j].vPosHeart(myTrack->fHeart) +
                          curSection->lNodes[j].vLatHeart(myTrack->fHeart) *
                              0.5f,
                      1.f);
        glVertex3f(curPos.x, curPos.y, curPos.z);
      }
    ...
    curPos =
        anchorBase *
        glm::vec4(curSection->lNodes[end].vPosHeart(myTrack->fHeart) +
                      curSection->lNodes[end].vLatHeart(myTrack->fHeart) * 0.5f,
                  1.f);
    glVertex3f(curPos.x, curPos.y, curPos.z);
    glEnd();
  }
